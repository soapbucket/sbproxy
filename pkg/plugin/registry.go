...
// mu guards all registry maps. A single RWMutex is used instead of per-map locks
// because registration happens only at init() time (write-heavy startup phase)
// and lookups happen during config compilation (read-heavy, infrequent).
// The maps store factory functions, not instances, so each origin gets its own
// module instance with independent state.
var (
	mu             sync.RWMutex
