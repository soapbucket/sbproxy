...
|  | scrub | Contains Wave/planning codes (W1-W6, A codes); strip to keep core a2a spec |
|  | scrub | Audit log schema is OSS, but dense with Wave 1-6 planning vocab and enterprise references (S3, Postgres batches, signed-log v0). Strip planning vocab, keep envelope shape |
|  | scrub | 31 Wave refs, mostly planning vocabulary. Core taxonomy is OSS-relevant; strip timeline refs |
|  | scrub | Dynamic client registration (OSS-relevant); heavy planning vocab. Strip Wave refs |
|  | scrub | Agent identity feed is OSS infrastructure. Strip Wave 2/G2.1 refs, keep spec |
|  | **delete** | Exclusively about enterprise signed-batch audit log (Wave 2), S3 Object Lock, Postgres batches, KMS, HKDF daily rotation. Pure enterprise feature |
|  | scrub | Layering rule is architecture; strip enterprise rail references. Keep async layering pattern |
|  | **delete** | Exclusively x402 + MPP (commercial-only payment rails). Entire document is Wave 3 rail mapping |
|  | keep | Bot authentication is OSS feature; minimal planning vocab (8 Wave refs, mostly dates) |
|  | scrub | Rate-limit schema is OSS; 23 planning refs. Strip timeline vocabulary |
|  | scrub | Content shape negotiation is OSS. Pricing and Wave 4 code (G4.1) are commercial; strip pricing-specific sections. Keep negotiation logic |
|  | **delete** | Exclusively about multi-rail refund/dispute model (x402, MPP, Stripe). Pure Wave 3 commercial feature |
|  | keep | Custom headers/properties are core proxy feature; minimal planning vocab (1 ref) |
|  | scrub | Schema migration is OSS concern. References Wave 1-6 timeline and enterprise features (Postgres audit log, S3). Strip timeline, keep migration pattern |
|  | scrub | Retention policy is relevant to any deployment. Heavy Wave refs (21); strip planning vocab |
|  | scrub | Idempotency across stack is OSS-relevant. 39 Wave refs and cross-rail orchestration. Strip rail-specific sections, keep idempotency-key pattern |
|  | keep | Event envelope is core observability; minimal planning vocab (4 refs) |
|  | **delete** | Exclusively x402 on-chain reorg handling (Wave 3, A3.3). No OSS relevance; commercial chain settlement only |
|  | scrub | Ledger protocol is OSS-relevant (the trait). 17 Wave refs. Strip planning timeline, keep wire spec |
|  | keep | JSON schema is core; 36 Wave refs all in context section. Strip context, keep schema |
|  | scrub | Log redaction is OSS feature. 20 Wave refs in timeline. Strip planning vocab |
|  | keep | Metric cardinality management is core observability; 10 Wave refs in planning sections |
|  | scrub | Classifier is OSS (ONNX-backed); 31 Wave refs, all timeline/planning. Strip planning vocab |
|  | scrub | 402 challenge wire format appears OSS (multi-rail negotiation), but deeply tied to x402/MPP commercial rails. Strip to keep single-rail 402 baseline; mark for potential rewrite since Wave 3 focused |
|  | scrub | Core observability; 26 Wave refs but concentrated in timeline/roadmap. Strip roadmap, keep metrics schema |
|  | scrub | Projections (robots.txt, llms.txt, licenses.xml) are OSS; 27 Wave refs and 4 enterprise refs (PDF transform). Strip Wave/enterprise, keep projection contract |
|  | scrub | Quote tokens are tied to commercial payment rails (Wave 3, A3.2). 27 Wave refs. Keep token schema as it's needed for the accept_payment policy, but strip rail-specific context |
|  | scrub | Schema versioning policy is OSS-relevant. 20 Wave refs, 6 enterprise refs. Strip timeline, keep versioning rules |
|  | keep | Session ID is core request tracking; minimal planning vocab (1 ref) |
|  | scrub | KYA (Know-Your-Agent) is OSS policy module. 38 Wave refs, 2 enterprise refs (attestation). Strip planning/enterprise refs |
|  | keep | Observability alerting is core; 24 Wave refs but in roadmap context. Categorization is OSS-relevant |
|  | keep | TLS fingerprinting is core security; 23 Wave refs in timeline |
|  | keep | User identity is core auth; minimal planning vocab (1 ref) |
|  | **delete** | Exclusively metered billing wallet (Wave 2, A2.1). Stripe integration, journaling for refunds/disputes. Pure enterprise commercial feature |
|  | scrub | Webhook signature verification is OSS (for inbound webhooks). 14 Wave refs, 2 enterprise refs (Stripe webhook). Strip planning vocab and Stripe-specific sections |
...
|  | scrub | Operational runbooks are partially OSS (auth, rate limits, cache, TLS) and partially commercial (rails, wallet, audit batches, merchant merchant disputes). 88 Wave refs, 10 enterprise refs. Heavy filtering needed; extract OSS-only runbook sections or consider deletion |
...
|  | scrub | Rate limiting is OSS; budgets are partially OSS (token/cost) and partially commercial. 18 Wave refs. Strip commercial budget-enforcement sections |
...
| runbooks/ | keep | Synthetic monitoring is OSS; no commercial content |
| scripting.md | keep | CEL/Lua/WASM scripting is OSS; no commercial content |
|  | scrub | Threat model is relevant to any deployment. 49 Wave refs, 10 enterprise refs (merchant disputes, x402 settlement, audit log tampering). Strip enterprise threat scenarios, keep proxy threats |
...
1. **** - Currently focused on Wave 3 multi-rail (x402 + MPP). OSS ships single-rail 402 only; this document needs comprehensive rewrite to document the single-rail baseline and clearly mark multi-rail as commercial-only future work.
...
3. **** - Primarily focused on commercial rail testing (x402, MPP, Stripe). OSS section (HTTP ledger) is minimal. Consider whether keeping as commercial-only runbook or deleting entirely is clearer.

4. **** - Heavily mixed OSS and enterprise. Merchant disputes, wallet settlement, audit batch DLQ, x402 reorg simulation dominate. Requires surgical extraction of OSS-only sections (auth, cache, TLS) or deletion.
...
2. **Merchant/commercial context leakage**: Several docs (, degradation.md, comparison.md, , payment-rails.md, ) assume a "SaaS Soap Bucket" merchant context (disputes, chargebacks, hosted ledger service, "Stripe outage pages on-call"). This vocabulary doesn't apply to OSS deployments. Recommend: strip merchant-specific examples and replace with self-hosted operator context.
...
6. **Schema lock (v1.0.x vs schema-v1) consistency**: Several ADRs and docs reference "schema-v1" (config schema version) alongside "v1.0.0" (binary version). Need to verify that references in , configuration.md, and upgrade.md correctly distinguish the two; confusion here could break OSS config migrations.
...
1.  - Heavy Wave refs (55); strip timeline, keep envelope schema
2.  - Heavy mixed (88 Wave, 10 enterprise); extract OSS sections or delete
3.  - Moderate Wave (39); idempotency pattern is OSS-relevant
4.  - Moderate Wave (45); rewrite to focus on single-rail baseline
5.  - Heavy enterprise context (10 refs); strip merchant/x402/audit tampering scenarios
