...
| `x-sbproxy-max-price` | Per-request price ceiling in USD on an `ai_proxy` origin. Only ever tightens the origin's `max_price_per_request`, never raises it. A value that is not a positive USD amount is refused with 400 rather than ignored, and so is one set on a surface with no pre-dispatch cost estimate. | `crates/sbproxy-core/src/server/ai_dispatch.rs` |
| `x-sbproxy-timeout-ms` | Per-request transport budget in milliseconds, replacing the selected provider's `timeout_ms` for this request. Honored only when the origin sets `allow_request_timeout_override: true`, and only up to `max_request_timeout_ms`. With the flag off the header is ignored rather than refused, counted as `ignored_override_disabled`. Above the ceiling, or not a positive whole number, it is refused with 400 `invalid_request_timeout` rather than clamped. Bounds one attempt, so `max_retries` multiplies it. | `crates/sbproxy-core/src/server/ai_dispatch.rs` |
