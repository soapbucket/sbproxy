...
On a streamed response, an enforcing `ai_tool_call` hook holds every tool-call frame back until its verdict lands, whether or not the hook rewrites anything. The verdict for a call that completes at the end of the turn arrives with the stop event, which is also the event the gateway turns into the stream terminator (`data: [DONE]` on OpenAI-shaped streams). The released frame is written ahead of that terminator, so a client that stops reading at the terminator still receives the call. Frames released earlier in the stream keep their arrival order. One combination does not compose: a mutating `ai_guardrail_output` hook holds the whole response back to close and then replaces it with its own text, so the replacement carries no tool calls and the released frame does not survive it. An output hook that rewrites the whole answer owns the whole answer. Pair `ai_tool_call` with an observing or blocking `ai_guardrail_output` hook, not a mutating one, when the turn has to keep its tool calls.
...
An inspect-only `ai_guardrail_input` hook may set `execution.mode: parallel` to run alongside the upstream call instead of before it. A block cancels the in-flight call. The allow path adds no extra time-to-first-token; a reject pays for one wasted generation, which `sbproxy_ai_parallel_moderation_total` records as `cancelled_upstream` when the hook won the race and `block` when the provider had already answered. A reject on a path that never dials a provider (semantic-cache hit, idempotency replay, or raced fan-out) is `refused`. Parallel cannot combine with `mutates: true` or `enforcement_mode: observe`. Serial remains the default.
