...
`client_disconnected` is written when delivery of a finished response to a proxied origin failed, and when an AI request was cancelled because the caller's connection broke; see [ai-gateway.md](ai-gateway.md#when-a-broken-connection-stops-the-meter). Three AI cases reach it, and they bill differently:

* **Cancelled before the response header.** The provider had reported nothing, so the receipt carries no units and `partial` and `yes` bill the same nothing.
* **Cancelled mid-stream.** The units are what the stream had delivered up to the failed write: the provider's own usage frame if one had arrived, this gateway's tokenizer count of the delivered text if not. These are non-zero, so `partial` and `yes` are different answers here and the table's choice decides real money.
* **A finished response the caller was no longer there to receive.** The write failure is attributed to the caller, so it prices as `client_disconnected` rather than as a delivered sale. Streaming and non-streaming responses both do this now.
...
The envelope is the chain link: `seq` is the entry's position, `prev_hash` is the previous entry's digest (all zeros for the first), `entry_hash` is the SHA-256 binding this entry to everything before it, and `signature` is the Ed25519 signature over that digest. The `event` inside is the receipt itself: the subject the units are charged to, the route, the outcome, the units with their provenance and raw evidence, the agreement they bill under, and `config_revision`, which identifies the set of origins the proxy was serving when it priced the call. Read that last field for what it is: it pins the routable generation a receipt was issued under, so receipts from two different origin sets cannot be conflated. It is not a hash of the configuration's contents and does not by itself answer "you priced my call under a rate I never agreed to": a change to a `route_weight` behind an unchanged hostname leaves it where it was. The number to check is the unit's `count`, which is the weight that was applied: compare it against the weight in the signed config bundle you agreed to. The revision then scopes the dispute to a serving generation; it does not prove the rate.
