...
`client_disconnected` is written both when delivery of a finished response failed and when an AI request was cancelled mid-generation because the caller's connection went away; see [ai-gateway.md](ai-gateway.md#a-client-that-hangs-up-stops-the-meter). The units on that second receipt are whatever the provider had reported, which for a call abandoned before its response header is none, so `partial` and `yes` bill the same nothing there.
