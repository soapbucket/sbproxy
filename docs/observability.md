...
| `sbproxy_key_operations_total` | 21 | Labels: `operation` (mint\|update\|delete\|revoke\|block\|unblock\|rotate), `outcome` (ok\|refused\|error). Every value is a compile-time constant chosen at the admin dispatch seam from the status class the handler actually returned, so the cap is 7 x 3 whatever the operator does. The three outcome values are never folded: `refused` is a 4xx the caller can fix (validation, revision conflict, terminal key), `error` is the store or governance backend failing. |
