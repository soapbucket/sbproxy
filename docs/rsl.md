...
[`examples/rsl-licensing/`](../examples/rsl-licensing/), on the host
`shop.localhost`. One `ai_crawl_control` policy plus the per-origin
`content_signal` field drives all three projections. Start it:

```bash
make run CONFIG=examples/rsl-licensing/sb.yml
```

The proxy serves each document off its in-memory projection cache, with no
Host or User-Agent filtering, so any caller can fetch them:

```bash
curl -sS -H 'Host: shop.localhost' http://127.0.0.1:8080/licenses.xml
```
...
<rsl xmlns="https://rslstandard.org/rsl" version="1.0">
  <content url="https://shop.localhost/*">
    <license urn="urn:rsl:1.0:shop.localhost:0">
...
```bash
curl -sS -H 'Host: shop.localhost' http://127.0.0.1:8080/.well-known/tdmrep.json
```
...
`tdm-policy` points back at `/licenses.xml`, which is how a TDM-aware crawler
gets from the reservation to the terms. One entry appears per priced path, so
the tiers in `ai_crawl_control` decide the shape of this array.

And a `robots.txt`, generated rather than authored:

```bash
curl -sS -H 'Host: shop.localhost' http://127.0.0.1:8080/robots.txt
```
...
Disallow: /docs/*
Crawl-delay: 1
# SBproxy-AI-Extension: pay-per-crawl price=0.002000 currency=USD shape=html
```
