...
- **Not hash-chained:** the `mcp_governance_decision` record itself, the
  ...
  chain above. Its own tamper-evidence mechanism is the gapless
  ...
  The sequence lives in proxy memory, so it is gapless per tenant **per
  emitting process**, and the record names that process in
  `sbproxy.evidence.instance`. A rule that groups on the tenant alone
  reads two replicas as one interleaved stream (`1, 1, 2, 2, 3, 3`, where
  neither a hole nor a duplicate is visible) and reads a restart as a
  rollback. Group on `(sbproxy.evidence.instance, sbproxy.tenant.id)`.
  The case no grouping recovers is a truncated tail: a replica killed
  mid-stream leaves a sequence that stops, and so does one shut down
  cleanly.

## The three governance bets, and what proves each one

- **Gapless, tamper-evident evidence.** `sbproxy.evidence.seq` never skips
  a number for a tenant on one emitting process while emission is enabled,
  and `sbproxy.evidence.instance` names that process, proven by
