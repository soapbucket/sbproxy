...
| Supplied ranking lacks a score | `skipped`, `missing_relevance_score` | The affected retrieval block is unchanged |
| Selection retains no chunks with `drop_empty: false` | `skipped`, `no_selected_chunks` | The affected retrieval block is unchanged |
| JSON is invalid, nested, heterogeneous, or otherwise unsafe for table encoding | `skipped`, `unsafe_structured_shape` | The unsafe chunk remains unchanged; other eligible chunks may still compact |
| Chunks already have the edge order | `skipped`, `already_ordered` | Chunk bytes and order remain unchanged |
