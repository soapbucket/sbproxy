...
| [`dlp-catalog`](../examples/dlp-catalog/) | Data Loss Prevention. | Use `dlp` policies for PII detection. | Detects sensitive shapes in the request URI and headers, then tags or blocks. `scan_body` defaults true, but the header-phase snapshot is empty, so a secret that appears only in the POST body is not seen. It does not mask or rewrite, and it has no response-scanning phase; use the `pii:` block on an `ai_proxy` origin to redact model traffic. |
