...
**Fallback:** `key_management.failure_posture` decides it, and all five inbound-key paths read the same value: the pre-auth header sweep, the playground impersonation ticket, the AI gateway's bearer path, the OIDC claim map, and native-provider-key admission. The default `closed` denies with `503`. `degraded` and `open` fall through to the origin's own configured auth rather than admitting outright, so an origin with a `credentials:` block still authenticates the caller; what is lost is the per-key policy, budget, and attribution the virtual key would have carried. `degraded` says so in the log; `open` does not. `observe` is rejected at config-compile time: a store that could not be read produced no verdict to record.

Native-provider-key admission has one added carve-out. If the store is unreachable and the posture has already decided to admit the request, a recognized provider credential with no admitting `native_key_policy` is not denied a second time: `failure_posture` already made the admission call, and overriding it here would contradict that decision. The request falls through without stamping governed native mode, because stamping it is what makes the rest of dispatch require a matching `accept_native_credentials_for` destination binding and refuse without one, which would reintroduce the same 403 one gate later under a different name. The provider is still recorded for observability, and the fall-through itself is logged at WARN.
