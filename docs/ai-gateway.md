...
| `provider` | string | unset | Pins the credential to one configured provider. A request or routing-policy plan that can only resolve to a different provider is not served: a cascade whose every tier names a locked-out provider is HTTP 502 (cascade exhaustion / credential lock), not 403. |
