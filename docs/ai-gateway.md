...
In addition to per-response cost headers, sbproxy exposes aggregate token usage via Prometheus metrics (available in the OSS edition). When `telemetry.bind_port` is configured, the following AI-specific counters are available at the `/metrics` endpoint under the `sbproxy_ai_*` namespace:

| Metric | Type | Labels | Description |
|--------|------|--------|-------------|
| `sbproxy_ai_requests_total` | Counter | `provider`, `model`, `status` | Total AI requests |
| `sbproxy_ai_tokens_input_total` | Counter | `provider`, `model` | Input tokens consumed |
| `sbproxy_ai_tokens_output_total` | Counter | `provider`, `model` | Output tokens generated |
| `sbproxy_ai_tokens_cached_total` | Counter | `provider`, `model` | Provider-side cached tokens |
| `sbproxy_ai_request_duration_seconds` | Histogram | `provider`, `model` | End-to-end AI request latency |
| `sbproxy_ai_ttft_seconds` | Histogram | `provider`, `model` | Time to first token (streaming only) |
| `sbproxy_ai_cache_hits_total` | Counter | | AI response cache hits |
| `sbproxy_ai_cache_misses_total` | Counter | | AI response cache misses |
| `sbproxy_ai_guardrail_triggers_total` | Counter | `type`, `action` | Guardrail blocks and flags |
| `sbproxy_ai_provider_errors_total` | Counter | `provider`, `error_type` | Provider-side errors |
| `sbproxy_ai_fallbacks_total` | Counter | `from_provider`, `to_provider` | Provider fallback events |

Use these metrics to build spending dashboards, set budget alerts, and track provider reliability without any application-level instrumentation.

## Provider Health Monitoring

The pre-built Grafana dashboard (`dashboards/grafana/sbproxy-ai-gateway.json`) visualizes AI gateway health in real time:

- **Provider availability** - Error rates and fallback frequency per provider
- **Token throughput** - Input/output token rates by provider and model
- **Latency** - Request duration and time-to-first-token histograms
- **Cache efficiency** - AI response cache hit ratio over time
- **Guardrail activity** - Block and flag rates by rule type
- **Cost tracking** - Estimated spend by provider and model

Import the dashboard into Grafana and point it at your Prometheus data source. See `dashboards/README.md` for provisioning instructions.
