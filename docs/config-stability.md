...
| `proxy.cache_reserve.backend.replication_target_bucket`, `.sse_kms_bucket_default` | Both belonged to the AWS-SDK cache-reserve backend, which is retired. The first was a diagnostics-only hint no code ever acted on: cross-region replication is an S3 bucket-level replication rule and an IAM role, configured outside sbproxy. The second chose between client-side envelope encryption and the bucket's own SSE-KMS, and the surviving backend needs no switch because SSE-KMS is a bucket setting that applies to whatever is written to it. | Nothing. The whole `type: s3` block that carries them is refused at config compile; see the row below. |
...
| `proxy.cache_reserve.backend.type: s3` | The AWS-SDK reserve backend was retired in favor of the object-storage one, which reaches the same buckets plus GCS, Azure Blob, a local directory, and any S3-compatible store an `endpoint` names, through one code path instead of two. The value stays in the vocabulary so the refusal can print the replacement block: this enum carries `#[serde(other)]`, so removing the variant would have made an existing block parse as an out-of-tree backend, load clean, serve, and leave the cold tier gone behind one startup warning. | `type: object_store` with `backend: s3`, keeping `bucket`, `region`, and `prefix` as written. `kms_key_id` has no equivalent: seal with `backend.encryption` inside the proxy, or leave encryption to the bucket's own SSE-KMS, which composes with either choice. The compile error prints the block; [cache-reserve.md](cache-reserve.md#migrating-from-type-s3) has the field-by-field table. |
