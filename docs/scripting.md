...
Two related surfaces are not in the table. `ai_policy.expression` evaluates over a single `ai` namespace and is documented in [ai-policy-cel.md](ai-policy-cel.md); it does not share this context, and its expressions are checked at config load against that one-namespace vocabulary rather than this table. An `mcp` action's `argument_policies[]` is the same shape, over a single `mcp` namespace (`mcp.tool.name`, `mcp.server`, `mcp.session.id`, `mcp.session.integrity`, `mcp.session.exfil_allowed`, `mcp.arguments`, `mcp.tenant`, `mcp.principal.{sub,team,project,user}`), evaluated against one call's tool-call context after RBAC and JSON-Schema validation pass; see [mcp-security.md](mcp-security.md). `mcp.session.integrity` and `mcp.session.exfil_allowed` are the deterministic session-flow labels the `flow` guardrail maintains (`trusted`/`tainted` and a sticky exfiltration-allowed bit); a rule reads them to compose a tighter policy than `flow`'s own built-in `outbound_tools` gate. Both `engine: cel` and `engine: rego` read the same context, so `mcp.tool.name` in CEL is `input.mcp.tool.name` in Rego.
