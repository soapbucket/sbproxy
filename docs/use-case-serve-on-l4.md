...
Exit code 2 means valid with changes present; a config that fails validation exits 3 with the findings printed. The serve-specific rejections are enforced at gateway start, before the listener takes traffic. An engine outside the allowlist (`unknown variant 'shell', expected one of 'auto', 'vllm', 'sglang', 'llama_cpp', 'mistralrs'`) or a `base_url` on a served provider is a fatal boot error with a message naming the fix. `validate` and `plan` going green is the part of this page that is true on every machine, GPU or none.
