...
| `circuit_breaker_open` | error | Circuit breaker opens (backend marked unhealthy) |
...
| Event | Severity | When |
|-------|----------|------|
| `clickhouse_down` | error | Analytics backend is unreachable |
| `clickhouse_up` | info | Analytics backend has recovered |
| `clickhouse_flush_success` | info | Analytics batch written successfully |
| `clickhouse_flush_error` | error | Analytics batch write failed |
| `clickhouse_max_retries_exceeded` | critical | Analytics write retries exhausted |
...
events.Subscribe(events.EventClickHouseMaxRetriesExceeded, func(event events.SystemEvent) error {
    return sendPagerDutyAlert(
        "Analytics write retries exhausted",
        event.Severity,
        event.Data,
    )
})
