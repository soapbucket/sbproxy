...
- `config_rollback` carries the `trigger` (`manual` or `auto_revert`), the `actor` when an operator asked, the `target` as it was named, the `outcome` (`applied` or `rejected`), and on a success both revisions, the appended revision, the blast radius, and whether the secrets fingerprint moved. A refusal carries the stable `reason` label and deliberately **not** the refusal's free text, which can quote the offending YAML. Both outcomes publish, because a refused rollback during an incident is exactly as interesting as an accepted one and a feed carrying only successes cannot answer "did anyone try".
