...
* **`abtest`** splits traffic across weighted backend variants for an A/B test; an existing sticky cookie pins a returning client, but the action never issues that cookie. Docs: [configuration.md](configuration.md#abtest), example: [A/B Test Routing](../examples/ab-test-routing/).
