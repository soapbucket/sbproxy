...
Sets the request log level independently from the application log. When empty, the request logger inherits from `--log-level`.
...
| `SB_REQUEST_LOG_LEVEL` | `--request-log-level` | (inherits) | Request log level (defaults to `SB_LOG_LEVEL`) |
