...
A Rego policy reads `input`, and `input` is the same assembled request context a CEL expression reads, converted to JSON. `request.trust_tier` in CEL is `input.request.trust_tier` in Rego. For AI traffic, the routing policy reads `input.ai`, the same vocabulary the CEL-based AI policy plane uses as an `ai.*` namespace; see [ai-policy-cel.md's namespace table](ai-policy-cel.md#the-ai-namespace) and the [Routing policy section of ai-gateway.md](ai-gateway.md#routing-policy) for the full field list rather than a copy of it here. Both engines are kept in sync by a parity test, so a binding available to one is available to the other.
