...
19. [ACME / Auto TLS](#acme--auto-tls)
20. [Redis Integration](#redis-integration)
...
origins:
  "api.example.com":
    ...
    traffic_capture: { ... }     # Optional: traffic capture config
    message_signatures: { ... }  # Optional: RFC 9421 signatures
    bot_detection: { ... }       # Optional: bot detection
    threat_protection: { ... }   # Optional: JSON/XML validation
```
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `type` | string | required | Must be `ip_filtering` |
...
sbproxy exposes Prometheus metrics and supports OpenTelemetry tracing via a dedicated telemetry server.

### Configuration

```yaml
telemetry:
  bind_port: 9090           # Port for metrics endpoint (default: disabled)
  bind_address: ""           # Bind address (default: all interfaces)
  enable_profiler: false     # Enable pprof profiler endpoints
```

### Prometheus Metrics

When `telemetry.bind_port` is set, sbproxy exposes metrics at `http://localhost:{port}/metrics`.

Key AI metrics (`sbproxy_ai_*` namespace):
- `requests_total` - Total AI requests by provider, model, status
- `tokens_input_total` / `tokens_output_total` - Token usage by provider/model
- `tokens_cached_total` - Provider-side cached tokens
- `request_duration_seconds` - AI request latency histogram
- `ttft_seconds` - Time to first token histogram (streaming)
- `cache_hits_total` / `cache_misses_total` - AI response cache
- `guardrail_triggers_total` - Guardrail blocks/flags by type
- `provider_errors_total` - Provider errors by type
- `fallbacks_total` - Provider fallback events

### Grafana Dashboards

Pre-built dashboards are in `dashboards/grafana/`:
- `sbproxy-overview.json` - Request rates, latency, errors, connections
- `sbproxy-ai-gateway.json` - AI provider health, tokens, TTFT, cache, guardrails
- `sbproxy-security.json` - WAF, rate limits, auth failures, bot detection
- `sbproxy-origins.json` - Per-origin breakdown

Import via Grafana UI or provisioning. See `dashboards/README.md` for setup.

### Prometheus Alerts

Pre-built alert rules in `dashboards/prometheus/alerts.yml`:
