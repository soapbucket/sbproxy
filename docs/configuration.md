...
A pod-local backend (`redb`, `sqlite`, `memory`) still falls back to in-memory, because a single node has nothing to be mutually excluded from. It is no longer quiet about it: the log line is at `error` and names the backend, and `sbproxy_cert_store_degraded{backend="..."}` goes to `1`. Alert on that. The cost is real, which is that every certificate is re-issued on every restart. The same gauge reads `0` when the configured backend opened, so an absent series is a proxy that never started rather than a healthy one.
