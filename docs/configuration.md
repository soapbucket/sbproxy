...
field and the complete batch is rejected when any entry fails.

Validation failures return `400` before an HTTP request is sent upstream.
Validated POST bodies are limited to 64 KiB so the accepted body can be
replayed unchanged after the pre-upstream check; larger bodies return
`413`. Multipart uploads and persisted-query-only envelopes are not
validated transports and therefore return `400` when any validation
control is enabled. They continue to pass through unchanged under the
default transparent configuration.
