...
| `usage_sinks` | `usage_sink`, `webhook` | Langfuse, Datadog, and object-store usage-sink deliveries (`usage_sink`), plus webhook usage-sink deliveries and the `events:` webhook sink (`webhook`, a separate purpose the same sub-block arms with one allowlist). `allow_private: true` also arms the events webhook SSRF host allowlist from `hosts` at boot; a SIGHUP cannot newly permit a private collector that was refused at start. See [events.md](events.md#where-the-batch-is-allowed-to-go). |
