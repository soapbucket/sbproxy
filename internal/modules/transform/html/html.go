// Package html registers the html transform.
package html
...
func init() {
	plugin.RegisterTransform("html", New)
}

// FormatOptions holds HTML format options.
...
// AttributeOptions holds HTML attribute options.
...
// Config holds configuration for the html transform.
type Config struct {
...
// htmlTransform implements plugin.TransformHandler.
type htmlTransform struct {
	tr transformer.Transformer
}

// New creates a new html transform.
func New(data json.RawMessage) (plugin.TransformHandler, error) {
	...
	return &htmlTransform{
		tr: transformer.ModifyHTML(modifyFns...),
	}, nil
}

func (h *htmlTransform) Type() string                    { return "html" }
func (h *htmlTransform) Apply(resp *http.Response) error { return h.tr.Modify(resp) }
