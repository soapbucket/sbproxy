...
-- Match hash
return fingerprint and string.sub(fingerprint.hash, 1, 5) == "v1.0:"

-- Check cookie count
return fingerprint and fingerprint.cookie_count > 5

-- Check connection duration
return fingerprint and fingerprint.conn_duration_ms < 100
```
...
return {
  add_headers = {
    ...
    ["X-Fingerprint"] = fingerprint and fingerprint.hash or "NONE"
  }
}
```
...
return {
  add_headers = {
    ...
    ["X-Fingerprint-Hash"] = fingerprint and fingerprint.hash or "NONE"
  }
}
```
...
return {
  add_headers = {
    ["X-Country"] = ipinfo and ipinfo.country_code or "XX",
...
### Fingerprint (may be nil)

```lua
fingerprint.hash                 -- Fingerprint hash
fingerprint.composite            -- Composite string
fingerprint.ip_hash              -- IP hash
fingerprint.user_agent_hash      -- UA hash
fingerprint.header_pattern       -- Header pattern
fingerprint.tls_hash             -- TLS fingerprint
fingerprint.cookie_count         -- Cookie count
fingerprint.conn_duration_ms     -- Connection duration
