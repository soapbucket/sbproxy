...
func newSandboxedState() *lua.LState {
	...
	// Replace os with a safe subset (time, clock, difftime only)
	safeOS := L.NewTable()
	origOS := L.GetGlobal("os")
	if tbl, ok := origOS.(*lua.LTable); ok {
		for _, name := range []string{"time", "clock", "difftime", "date"} {
			if fn := L.GetField(tbl, name); fn != lua.LNil {
				safeOS.RawSetString(name, fn)
			}
		}
	}
	...
	// Register utility functions (sb.log, sb.base64, sb.json, sb.crypto, sb.uuid, sb.time)
	RegisterUtilFunctions(L)
