...
func wrapJSONModifierScript(script string) string {
	trimmed := strings.TrimSpace(script)
	if strings.Contains(trimmed, "function modify_json") {
		return trimmed
	}
	return "function modify_json(data, ctx)\nlocal json = data\n" + trimmed + "\nend"
}

func isJSONModifierFunction(script string) bool {
	return strings.Contains(strings.TrimSpace(script), "function modify_json")
}
