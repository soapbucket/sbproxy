...
return {
  add_headers = {
    ...
    ["X-Country"] = geoip and geoip.country_code or "UNKNOWN"
  }
}
```
...
- `geoip.country` - Country name
- `geoip.country_code` - ISO country code (e.g., "US", "GB")
- `geoip.continent` - Continent name
- `geoip.continent_code` - Continent code (e.g., "NA", "EU")
- `geoip.asn` - Autonomous System Number
- `geoip.as_name` - AS organization name
- `geoip.as_domain` - AS domain
...
return {
  add_headers = {
    ...
    ["X-Request-Country"] = geoip and geoip.country_code or "UNKNOWN"
  }
}
