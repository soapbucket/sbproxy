...
return {
  add_headers = {
    ...
    ["X-Country"] = ipinfo and ipinfo.country_code or "UNKNOWN"
  }
}
```
...
Browser/client fingerprint information (table, may be nil):
...
### IP Info (`ipinfo`)
GeoIP and ASN information (table, may be nil):
- `ipinfo.country` - Country name
- `ipinfo.country_code` - ISO country code (e.g., "US", "GB")
- `ipinfo.continent` - Continent name
- `ipinfo.continent_code` - Continent code (e.g., "NA", "EU")
- `ipinfo.asn` - Autonomous System Number
- `ipinfo.as_name` - AS organization name
- `ipinfo.as_domain` - AS domain
...
return {
  add_headers = {
    ...
    ["X-Request-Country"] = ipinfo and ipinfo.country_code or "UNKNOWN"
  }
}
...
- ✅ JSON modification
- ✅ HTML token matching
- ✅ Context variables (fingerprint, user agent, location, session)
- ✅ Error handling
...
```bash
# Unit tests
go test ./internal/lua

# Benchmarks
go test ./internal/lua -bench=. -benchtime=1s

# Coverage
go test ./internal/lua -cover

# Verbose
go test ./internal/lua -v
```
