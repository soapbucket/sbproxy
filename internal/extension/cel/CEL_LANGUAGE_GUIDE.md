...
// Match hash
size(fingerprint) > 0 && fingerprint['hash'].startsWith('v1.0:')

// Check cookie count
size(fingerprint) > 0 && fingerprint['cookie_count'] > 5

// Check connection duration
size(fingerprint) > 0 && fingerprint['conn_duration_ms'] < 100
```
...
```cel
(size(fingerprint) > 0 && fingerprint['cookie_count'] == 0) ||
...
```cel
{
  "add_headers": {
    ...
    "X-Fingerprint": size(fingerprint) > 0 ? fingerprint['hash'] : "NONE"
  }
}
```
...
```cel
{
  "add_headers": {
    ...
    "X-Fingerprint-Hash": size(fingerprint) > 0 ? fingerprint['hash'] : "NONE"
  }
}
```
