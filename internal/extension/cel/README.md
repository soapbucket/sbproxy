...
Browser/client fingerprint information (may be `null` if not available):
- `fingerprint['hash']` - Unique fingerprint hash
- `fingerprint['composite']` - Composite fingerprint string
- `fingerprint['ip_hash']` - Hashed IP address
- `fingerprint['user_agent_hash']` - Hashed user agent
- `fingerprint['header_pattern']` - HTTP header pattern
- `fingerprint['tls_hash']` - TLS/JA3 fingerprint hash
- `fingerprint['cookie_count']` - Number of cookies
- `fingerprint['conn_duration_ms']` - Connection duration in milliseconds
...
// Match based on fingerprint
matcher, err := cel.NewMatcher(`
    fingerprint != null && 
    fingerprint['hash'].startsWith('v1.0:')
`)
...
- All request context variables (`request_ip`, `cookies`, `params`, `fingerprint`, `user_agent`, `location`, `session`, `oauth_user`)
...
expr := `{
    "add_headers": {
        "X-Fingerprint": fingerprint != null ? fingerprint['hash'] : "NONE"
    }
}`
...
All context variables (`fingerprint`, `user_agent`, `location`, `session`) are always available as empty maps if the data is not available for a particular request. CEL doesn't support direct null comparison with maps, so use `size()` to check if a map has data:
...
1. **Fingerprint**: Retrieved from `fingerprint.Get(req)` - browser/client fingerprinting data
2. **User Agent**: Retrieved from `uaparser.Get(req)` - parsed user agent information
3. **IP Info**: Retrieved from `maxmind.Get(req)` - GeoIP and ASN data
...
- `convertFingerprintToMap()` - Converts fingerprint struct to map
...
- `internal/fingerprint` - Browser/client fingerprinting
- `internal/uaparser` - User agent parsing
- `internal/maxmind` - GeoIP and ASN lookup
- `internal/session` - Session management
- `internal/contextdata` - Request context data storage
