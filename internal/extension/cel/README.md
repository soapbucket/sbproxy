...
CEL expressions have access to crypto, UUID, and time utility functions:
...
Computes the HMAC-SHA256 of data using the given key, returns hex-encoded digest:
```cel
hmacSHA256("hello", "secret")  // "88aab3ede8d3adf94d26ab90d3bafd4a2083070c3bcce9c014ee04a443847c0b"
```
...
### `now() -> timestamp`
Returns the current time as a CEL timestamp. Supports all CEL timestamp methods:
```cel
now()                       // current timestamp
now().getFullYear()         // current year (e.g., 2026)
now().getMonth()            // current month (0-11)
now().getDayOfWeek()        // day of week (0=Sunday)
now().getHours()            // current hour (0-23)
```
...
1. **User Agent**: Retrieved from `reqctx.GetRequestData(req.Context()).UserAgent` - parsed user agent information
2. **Location**: Retrieved from `reqctx.GetRequestData(req.Context()).Location` - GeoIP and ASN data
3. **Session**: Retrieved from context data with key "session" - session and authentication data

User agent and location data is populated by the enricher middleware (`internal/engine/middleware/enricher.go`), which calls registered `plugin.RequestEnricher` implementations. GeoIP and UA parsing enrichers are available in sbproxy-enterprise.
...
- `internal/request/reqctx` - Request context data storage (UserAgent, Location, Session)
- `internal/request/session` - Session management
- `internal/engine/middleware/enricher.go` - Enricher middleware that populates user agent and location data
- `pkg/plugin` - RequestEnricher interface for extensible enrichment
