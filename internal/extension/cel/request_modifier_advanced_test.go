...
func TestRequestModifier_URLModifications(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			modifier, err := NewModifier(tt.expr)
			if err != nil {
...
func TestRequestModifier_PathModifications(t *testing.T) {
	tests := []struct {
		name     string
		expr     string
		reqPath  string
		...
		{
			name:     "path suffix",
			expr:     `{"path_suffix": ".json"}`,
			reqPath:  "/users",
			wantPath: "/users.json",
		},
		{
			name:     "path replace",
			expr:     `{"path_replace": {"/api/v1/": "/api/v2/"}}`,
			reqPath:  "/api/v1/users",
			wantPath: "/api/v2/users",
		},
		{
			name:     "path prefix and suffix",
			expr:     `{"path_prefix": "/api", "path_suffix": ".json"}`,
			reqPath:  "/users",
			wantPath: "/api/users.json",
		},
		{
			name:     "path replace then prefix",
			expr:     `{"path_replace": {"old": "new"}, "path_prefix": "/v2"}`,
			reqPath:  "/old/path",
...
func TestRequestModifier_QueryModifications(t *testing.T) {
	tests := []struct {
		name      string
		expr      string
		reqQuery  string
...
func TestRequestModifier_BodyModifications(t *testing.T) {
	tests := []struct {
		...
		{
			name:     "body remove",
			expr:     `{"body_remove": true}`,
			wantBody: "",
			wantCT:   "",
		},
		{
			name:     "body replace",
			expr:     `{"body_replace": "new body content"}`,
			wantBody: "new body content",
			wantCT:   "",
		},
		{
			name:     "body replace JSON",
			expr:     `{"body_replace_json": "{\"key\": \"value\"}"}`,
			wantBody: `{"key": "value"}`,
			wantCT:   "application/json",
		},
		{
			name:     "body replace base64",
			expr:     `{"body_replace_base64": "` + base64.StdEncoding.EncodeToString([]byte("decoded content")) + `"}`,
...
func TestResponseModifier_StatusTextModification(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			expr:       `{"status_code": 200, "status_text": "Custom Success"}`,
			wantStatus: "200 Custom Success",
		},
		{
			name:       "status text only",
			expr:       `{"status_text": "Custom Message"}`,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			modifier, err := NewResponseModifier(tt.expr)
			if err != nil {
...
func TestResponseModifier_BodyModifications(t *testing.T) {
	tests := []struct {
		...
		{
			name:     "body replace",
			expr:     `{"body_replace": "new response"}`,
			wantBody: "new response",
			wantCT:   "",
		},
		{
			name:     "body replace JSON",
			expr:     `{"body_replace_json": "{\"status\": \"success\"}"}`,
			wantBody: `{"status": "success"}`,
			wantCT:   "application/json",
		},
		{
			name:     "body replace base64",
			expr:     `{"body_replace_base64": "` + base64.StdEncoding.EncodeToString([]byte("decoded")) + `"}`,
			wantBody: "decoded",
			wantCT:   "",
		},
	}
