...
// Benchmark JSON Modifiers
func BenchmarkJSONModifierSimple(b *testing.B) {
	b.ReportAllocs()
	modifier, _ := NewJSONModifier(`{"set_fields": {"new_field": "value"}}`)
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_, _ = modifier.ModifyJSON(jsonObj)
	}
}

func BenchmarkJSONModifierComplex(b *testing.B) {
	b.ReportAllocs()
	modifier, _ := NewJSONModifier(`{
		"set_fields": {"new_field": "value", "another": 123},
