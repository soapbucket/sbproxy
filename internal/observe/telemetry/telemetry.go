...
var (
	certMgr   *fingerprint.CertManager
...
func Initialize(c Config, ctx context.Context, configDir string) error {
	...
	if certificateFile != "" && certificateKeyFile != "" {
		keyPairs := []fingerprint.TLSKeyPair{
			{
				Cert: certificateFile,
				Key:  certificateKeyFile,
			},
		}
		mgr, err := fingerprint.NewCertManager(keyPairs, configDir, "telemetry")
		if err != nil {
		...
		tlsConfig := &tls.Config{
			GetCertificate:           certMgr.GetCertificateFunc(fingerprint.DefaultTLSKeyPairID),
			MinVersion:               fingerprint.GetTLSVersion(c.MinTLSVersion),
			NextProtos:               []string{"h2", "http/1.1"},
			CipherSuites:             fingerprint.GetTLSCiphersFromNames(c.TLSCipherSuites),
