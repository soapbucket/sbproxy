...
//
// Architecture: Events are published to a bounded channel buffer and consumed by
// a pool of dispatcher goroutines. Each dispatcher reads events from the shared
// channel and invokes registered handlers. Per-workspace semaphores ensure that
// a noisy tenant cannot starve other workspaces of handler capacity.
package events
...
//
// Concurrency model:
//   - handlerSem (global, 32 slots) caps total concurrent handler goroutines.
//   - perWorkspaceSem (per-workspace, 32 slots each) provides tenant isolation
//     so one workspace cannot exhaust all handler slots.
//   - Both semaphores must be acquired before a handler runs; released on completion.
type InProcessEventBus struct {
	...
	stopCh     chan struct{}
	wg         sync.WaitGroup
	handlerSem chan struct{} // Global concurrency limiter for handler goroutines
