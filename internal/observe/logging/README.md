...
Security events capture authentication, authorization, threat detection, and other security-related activities.
...
// Threat detection
logging.LogThreatDetected(ctx, threatType, ip, details)
...
Preserves nested group structure for Elasticsearch indexing.

**Key Features**:
- Preserves nested groups for better Elasticsearch structure
- Includes caller information when `AddSource: true`
- Outputs structured JSON suitable for Logstash processing
...
- Uses relative paths from project root (`internal/` or `lib/`)
...
- `clickhouse_handler.go`: Handler for request logs (ClickHouse)
- `logstash_handler.go`: Handler for security/application logs (Elasticsearch)
