...
```bash
go test -v ./internal/platform/storage/sqlite/...
```
...
```bash
cd internal/platform/storage/postgres
./test-with-docker.sh
```
...
# Run tests
go test -v ./internal/platform/storage/postgres/...

# Or with custom DSN
POSTGRES_TEST_DSN="postgres://user:pass@host:port/db?sslmode=disable" go test -v ./internal/platform/storage/postgres/...
...
```bash
go test -v ./internal/platform/storage/...
```
