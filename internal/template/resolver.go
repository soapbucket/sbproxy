...
// Compiled templates are cached in a sync.Map keyed by template string. Since
// templates are config-derived and finite, the cache grows to a bounded size
// and never needs eviction.
type Resolver struct {
	cache sync.Map // map[string]*mustache.Template
}

var globalResolver = newResolver()

// serverVarsGetter is a callback to retrieve server variables. Registered by the
// service layer at startup to break the import cycle: template -> servervar -> template.
...
func (r *Resolver) getTemplate(templateStr string) (*mustache.Template, error) {
	...
	// forceRaw=true disables HTML escaping. The proxy produces headers, URLs, and
	// API responses where HTML escaping would corrupt values like query strings.
...
// client, ctx, secrets) plus utility variables (timestamp, date, uuid, random).
// Each namespace falls back gracefully when its data source is nil.
