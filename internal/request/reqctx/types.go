...
func (r *RequestData) Release() {
	...
	// If a map grew beyond 64 entries from a single request, nil it out
	// instead of clearing so the oversized backing array is not retained in the pool.
	r.DebugHeaders = clearOrNilMap(r.DebugHeaders)
	r.Config = clearOrNilMap(r.Config)
	r.Secrets = clearOrNilMap(r.Secrets)
	r.Variables = clearOrNilMap(r.Variables)
	r.Env = nil
	r.FeatureFlags = clearOrNilMap(r.FeatureFlags)
	r.Data = clearOrNilMap(r.Data)
	r.lowercaseHeaders = clearOrNilMap(r.lowercaseHeaders)
	r.lowercaseOrigHeaders = clearOrNilMap(r.lowercaseOrigHeaders)
...
// Headers and URL are captured lazily on first access via sync.Once to avoid
// allocations on simple proxy requests that never inspect request metadata.
type OriginalRequestData struct {
	Method      string      `json:"method"`
	url         string      // lazily computed from req; use GetURL()
	Path        string      `json:"path"`
	RawQuery    string      `json:"raw_query"`
	headers     http.Header // lazily cloned from req; use GetHeaders()
	...
	// req holds a reference to the original *http.Request for lazy field computation.
	// Only used during the lifetime of the request; cleared on Release.
...
// GetHeaders returns the cloned original request headers, computing them lazily on first access.
func (o *OriginalRequestData) GetHeaders() http.Header {
	if o == nil {
		return nil
	}
	o.headersOnce.Do(func() {
		if o.req != nil {
			o.headers = CloneHeader(o.req.Header)
		}
	})
	return o.headers
}

// GetURL returns the original request URL string, computing it lazily on first access.
func (o *OriginalRequestData) GetURL() string {
	if o == nil {
		return ""
	}
	o.urlOnce.Do(func() {
		if o.req != nil {
			o.url = o.req.URL.String()
		}
	})
	return o.url
}

// SetRequest stores the original HTTP request reference for lazy field computation.
// Must be called before any consumer accesses GetHeaders() or GetURL().
func (o *OriginalRequestData) SetRequest(r *http.Request) {
	o.req = r
}

// MarshalJSON implements json.Marshaler so that lazily-computed fields
// (headers, url) are included in the JSON output when marshaled.
func (o *OriginalRequestData) MarshalJSON() ([]byte, error) {
	type alias struct {
		Method      string      `json:"method"`
		URL         string      `json:"url"`
		Path        string      `json:"path"`
...
func (o *OriginalRequestData) Release() {
	...
	o.url = ""
	o.req = nil
	// Reset sync.Once values so pooled objects work correctly on reuse.
	o.headersOnce = sync.Once{}
	o.urlOnce = sync.Once{}
...
// maxPoolMapCap is the threshold above which pooled maps are nilled out on Release
// instead of cleared, to prevent oversized backing arrays from persisting in the pool.
const maxPoolMapCap = 64

// clearOrNilMap clears a map if small, or nils it if it grew beyond maxPoolMapCap.
func clearOrNilMap[V any](m map[string]V) map[string]V {
	if m == nil {
		return nil
	}
	if len(m) > maxPoolMapCap {
		return nil
	}
	clear(m)
	return m
