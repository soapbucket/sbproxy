...
// RequestContextV2 is the new unified request context that replaces RequestData.
// It holds references to the 9 context objects for the new namespace model.
// Named V2 during migration to avoid collision with the existing RequestData.
...
// ReleaseV2 returns the RequestContextV2 to the pool.
func (rc *RequestContextV2) ReleaseV2() {
	...
	clear(rc.lowercaseHeaders)
	clear(rc.lowercaseOrigHeaders)
...
func BuildRequestSnapshot(ord *OriginalRequestData) *RequestSnapshot {
	...
	if ord.Headers != nil {
		rs.Headers = headersToLowercaseMap(ord.Headers)
	}
