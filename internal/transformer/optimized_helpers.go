...
// OptimizedReplaceContent replaces the text content inside matching tags.
// It tracks tag open/close state so that only text nodes inside a matched
// element are replaced.
func OptimizedReplaceContent(matcher *reqctx.Matcher, newContent string) OptimizedModifyFn {
	var depth int // tracks nesting depth inside matched tags

	return func(token reqctx.SimpleToken, writer io.Writer) error {
		switch token.Type {
		case reqctx.StateTagName:
			if strings.HasPrefix(token.TagName, "/") {
				// End tag: leave matched context if depth > 0
				if depth > 0 {
					depth--
				}
			} else if matcher.MatchOptimized(token) {
				// Start tag matches: enter matched context
				depth++
			}
		case reqctx.StateText:
			if depth > 0 {
				// Inside a matched element: replace the text content
