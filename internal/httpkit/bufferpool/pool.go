...
type AdaptiveBufferPool struct {
	mu          sync.RWMutex
	tiers       []*BufferTier
	sizeHistory []int // Track requested sizes for analysis
	historySize int
	historyIdx  int
...
func NewAdaptiveBufferPool(config AdaptiveBufferPoolConfig) *AdaptiveBufferPool {
	...
	pool := &AdaptiveBufferPool{
		...
		sizeHistory:    make([]int, config.HistorySize),
...
func (p *AdaptiveBufferPool) recordSize(size int) {
	...
	p.sizeHistory[p.historyIdx] = size
	p.historyIdx = (p.historyIdx + 1) % p.historySize
}
...
func (p *AdaptiveBufferPool) AdjustSizes() {
	...
	history := make([]int, len(p.sizeHistory))
	copy(history, p.sizeHistory)
	p.historyMu.Unlock()
	
	// Filter out zeros (empty slots)
	var sizes []int
	for _, size := range history {
		if size > 0 {
