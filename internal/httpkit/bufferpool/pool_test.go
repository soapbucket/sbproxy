...
func TestAdaptiveBufferPool_RecordSize(t *testing.T) {
	...
	// Check that sizes were recorded (atomic ring buffer, no lock needed)
	recorded := 0
	for i := 0; i < pool.historySize; i++ {
		if pool.sizeHistory[i].Load() > 0 {
			recorded++
		}
	}

	if recorded == 0 {
		t.Error("expected at least some sizes to be recorded")
	}
}
