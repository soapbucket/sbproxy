...
import (
	...
	"github.com/soapbucket/sbproxy/internal/request/maxmind"
...
func NewManager(ctx context.Context, settings GlobalSettings) (Manager, error) {
	...
	// Initialize maxmind (use embedded ipinfo_lite.mmdb as default)
	if settings.MaxmindSettings.Driver == "" {
		mmdbPath := embedded.FilePath("ipinfo_lite.mmdb", "")
		if mmdbPath != "" {
			settings.MaxmindSettings.Driver = "maxmind"
			if settings.MaxmindSettings.Params == nil {
				settings.MaxmindSettings.Params = make(map[string]string)
			}
			settings.MaxmindSettings.Params["path"] = mmdbPath
		} else {
			slog.Warn("no maxmind/geoip driver configured; geo-based routing, IP country filtering, and location headers will not work")
		}
	}
	var maxmind maxmind.Manager
	maxmind, err = initMaxMind(settings.MaxmindSettings)
	if err != nil {
		slog.Warn("failed to initialize maxmind", "error", err)
		err = nil
	}
	m.maxmind = maxmind

	// Initialize uaparser (use embedded regexes.yml as default)
	if settings.UAParserSettings.Driver == "" {
		regexPath := embedded.FilePath("regexes.yml", "")
		if regexPath != "" {
			settings.UAParserSettings.Driver = "uaparser"
			if settings.UAParserSettings.Params == nil {
				settings.UAParserSettings.Params = make(map[string]string)
			}
			settings.UAParserSettings.Params["path"] = regexPath
		} else {
			slog.Warn("no uaparser driver configured; user-agent parsing and device detection will not work")
		}
	}
	var uaparser uaparser.Manager
	uaparser, err = initUAParser(settings.UAParserSettings)
	if err != nil {
		slog.Warn("failed to initialize uaparser", "error", err)
		err = nil
	}
	m.uaparser = uaparser
...
type managerImpl struct {
	...
	maxmind   maxmind.Manager
	uaparser  uaparser.Manager
...
func (m *managerImpl) Close() error {
	...
	// Close maxmind
	if m.maxmind != nil {
		if err := m.maxmind.Close(); err != nil {
			slog.Error("error closing maxmind", "error", err)
			errors = append(errors, err)
		}
	}
	...
	// Close uaparser
	if m.uaparser != nil {
		if err := m.uaparser.Close(); err != nil {
			slog.Error("error closing uaparser", "error", err)
			errors = append(errors, err)
		}
	}
...
func initMaxMind(settings maxmind.Settings) (maxmind.Manager, error) {
	return maxmind.NewManager(settings)
}

func initUAParser(settings uaparser.Settings) (uaparser.Manager, error) {
	return uaparser.NewManager(settings)
}
