...
func NewManager(ctx context.Context, settings GlobalSettings) (Manager, error) {
	...
	// Initialize geoip (use embedded geoip_country.mmdb as default, if present)
	if settings.GeoIPSettings.Driver == "" {
		mmdbPath := embedded.FilePath("geoip_country.mmdb", "")
		if mmdbPath != "" {
			settings.GeoIPSettings.Driver = "geoip"
			if settings.GeoIPSettings.Params == nil {
				settings.GeoIPSettings.Params = make(map[string]string)
			}
			settings.GeoIPSettings.Params["path"] = mmdbPath
		} else {
			slog.Warn("no geoip driver configured; geo-based routing, IP country filtering, and location headers will not work")
		}
	}
	geoipMgr, geoipErr := initGeoIP(settings.GeoIPSettings)
	if geoipErr != nil {
		slog.Warn("failed to initialize geoip", "error", geoipErr)
	}
	m.geoip = geoipMgr
	...
	uaparserMgr, uaparserErr := initUAParser(settings.UAParserSettings)
	if uaparserErr != nil {
		slog.Warn("failed to initialize uaparser", "error", uaparserErr)
	}
	m.uaparser = uaparserMgr
...
func (m *managerImpl) Close() error {
	...
	if m.geoip != nil {
		if err := m.geoip.Close(); err != nil {
			slog.Error("error closing geoip", "error", err)
			errors = append(errors, err)
		}
	}
...
func initGeoIP(settings geoip.Settings) (geoip.Manager, error) {
	return geoip.NewManager(settings)
}
