...
func TestPurgeOriginCache_Unit(t *testing.T) {
	...
	for _, hostname := range hostnames {
		if _, ok := defaultLoader.cache.Get(hostname); !ok {
	...
	if _, ok := defaultLoader.cache.Get("child.test"); ok {
...
func TestFindRelatedHostnames(t *testing.T) {
	...
	snapshot := defaultLoader.cacheSnapshot()
...
func TestHandleMessage(t *testing.T) {
	...
	// Verify cached
	if _, ok := defaultLoader.cache.Get("test.test"); !ok {
