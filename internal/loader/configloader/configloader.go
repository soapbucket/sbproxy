...
func LoadFallbackConfig(ctx context.Context, req *http.Request, fallback *config.FallbackOrigin, m manager.Manager, parent *config.Config) (*config.Config, error) {
	...
	if fallback.HasEmbeddedOrigin() {
		cfg, err := loadEmbeddedConfig(fallback.Origin, parent, m)
		if err != nil {
	...
	cfg, err := getConfigByHostname(ctx, req, fallback.Hostname, 0, m, parent)
	if err != nil {
...
// dedicatedWorkspaceID restricts this proxy to serving a single workspace.
var dedicatedWorkspaceID string

// SetWorkspaceID configures this proxy instance to serve only one workspace.
func SetWorkspaceID(id string) {
	dedicatedWorkspaceID = id
}

// drainedWorkspaces tracks workspaces being drained from this instance.
var drainedWorkspaces sync.Map
...
func cacheConfig(key string, cfg *config.Config) {
...
func checkHostFilter(hostname, normalizedHost string) *config.Config {
	if hostFilter == nil || normalizedHost == "" {
		return nil
	}
	if !hostFilter.Check(normalizedHost) {
...
func checkWorkspaceAccess(hostname string, cfg *config.Config) *config.Config {
	// In dedicated mode, reject configs that belong to a different workspace
	if dedicatedWorkspaceID != "" && cfg.WorkspaceID != "" && cfg.WorkspaceID != dedicatedWorkspaceID {
		slog.Debug("rejecting request for different workspace in dedicated mode",
			"request_workspace", cfg.WorkspaceID,
			"dedicated_workspace", dedicatedWorkspaceID,
			"hostname", hostname)
		return &config.Config{
			Hostname: hostname,
	...
	if cfg.WorkspaceID != "" {
		if _, draining := drainedWorkspaces.Load(cfg.WorkspaceID); draining {
...
func enrichRequestData(ctx context.Context, cfg *config.Config, rd *reqctx.RequestData, req *http.Request, m manager.Manager) {
	...
	if cfg.FallbackOrigin != nil {
		cfg.FallbackLoader = func(ctx context.Context, req *http.Request, fallback *config.FallbackOrigin) (*config.Config, error) {
			return LoadFallbackConfig(ctx, req, fallback, m, cfg)
		}
	}
...
func Load(req *http.Request, m manager.Manager) (*config.Config, error) {
	...
	if rejected := checkHostFilter(req.Host, normalizedHost); rejected != nil {
		return rejected, nil
	}

	cfg, err := getConfigByHostname(req.Context(), req, req.Host, 0, m, nil)
	if err != nil {
		return nil, err
	}

	if rejected := checkWorkspaceAccess(req.Host, cfg); rejected != nil {
		return rejected, nil
	}
	...
	enrichRequestData(ctx, cfg, requestData, req, m)
...
func enrichWithNewContext(cfg *config.Config, rd *reqctx.RequestData, req *http.Request) {
	// OriginContext - built once per config, shared across requests
	originCtx := cfg.OriginCtx()
	// Update params/secrets on the origin context (they may have been loaded after initial build)
	...
	rd.ServerCtx = config.GetServerContext()
...
func LoadForProxyHost(ctx context.Context, auth *ProxyAuthResult, hostname string, m manager.Manager) (*config.Config, error) {
	...
	return LoadForProxyRequest(req, auth.WorkspaceID, m)
}
...
func tryFailsafeResolution(ctx context.Context, hostname string, parent *config.Config, m manager.Manager, eventType, reason, detail string) *config.Config {
	if failsafeCfg, err := loadFailsafeSnapshotConfig(ctx, hostname, parent, m, reason, detail); err == nil {
	...
	if explicitCfg, err := loadExplicitFailsafeOriginConfig(ctx, hostname, parent, m); err == nil {
...
func applyForwardRules(ctx context.Context, req *http.Request, cfg *config.Config, hostname string, depth int, m manager.Manager, label string) (*config.Config, error) {
	...
	if matched.Hostname != "" {
		...
		return getConfigByHostname(ctx, req, matched.Hostname, depth+1, m, cfg)
	}
...
func getConfigByHostname(ctx context.Context, req *http.Request, hostname string, depth int, m manager.Manager, parent *config.Config) (*config.Config, error) {
	...
	if ok {
		...
		if fwdCfg, fwdErr := applyForwardRules(ctx, req, cfg, hostname, depth, m, " (cached)"); fwdCfg != nil || fwdErr != nil {
	...
	if err != nil && !errors.Is(err, storage.ErrKeyNotFound) {
		...
		if fs := tryFailsafeResolution(ctx, hostname, parent, m, "config.source_unavailable", configReasonSourceUnavailable, err.Error()); fs != nil {
			return fs, nil
		}
		return nil, err
	}
	...
	if data == nil {
		...
		if fs := tryFailsafeResolution(ctx, hostname, parent, m, "config.not_found", configReasonLoadFailed, "hostname not in storage"); fs != nil {
			return fs, nil
		}
		...
		if m.GetGlobalSettings().OriginLoaderSettings.HostnameFallback {
			...
			if hostnameWithoutPort, _, _ := net.SplitHostPort(hostname); hostnameWithoutPort != hostname && hostnameWithoutPort != "" {
				...
				return getConfigByHostname(ctx, req, hostnameWithoutPort, depth, m, parent)
			}
		}
		...
		if dotIdx > 0 && dotIdx < len(hostname)-1 {
			...
			if wildcardErr == nil && wildcardData != nil {
				...
				if parseErr != nil {
					...
					if failsafeCfg, err := loadFailsafeSnapshotConfig(ctx, wildcardHostname, parent, m, configReasonWildcardFallback, parseErr.Error()); err == nil {
					...
					if explicitCfg, explicitErr := loadExplicitFailsafeOriginConfig(ctx, wildcardHostname, parent, m); explicitErr == nil {
					...
					configureLoadedConfig(wildcardCfg, m)
					saveFailsafeSnapshot(wildcardHostname, wildcardCfg, wildcardData)
					// Cache with workspace partition (use hostname as key if no workspace)
					cacheConfig(hostname, wildcardCfg)
	...
	if err != nil {
		...
		if failsafeCfg, fallbackErr := loadFailsafeSnapshotConfig(ctx, hostname, parent, m, classifyFailureReason(err), err.Error()); fallbackErr == nil {
		...
		if explicitCfg, explicitErr := loadExplicitFailsafeOriginConfig(ctx, hostname, parent, m); explicitErr == nil {
			eventType := "config.validation_failed"
			if strings.Contains(strings.ToLower(err.Error()), "compile") || strings.Contains(strings.ToLower(err.Error()), "lua") || strings.Contains(strings.ToLower(err.Error()), "cel") {
				eventType = "config.compile_failed"
			}
	...
	configureLoadedConfig(cfg, m)
	saveFailsafeSnapshot(hostname, cfg, data)
	...
	if fwdCfg, fwdErr := applyForwardRules(ctx, req, cfg, hostname, depth, m, ""); fwdCfg != nil || fwdErr != nil {
		return fwdCfg, fwdErr
	}
...
func configureLoadedConfig(cfg *config.Config, m manager.Manager) {
	...
	// OriginConfigLoader resolves an existing origin by hostname.
	cfg.OriginConfigLoader = func(hostname string) (http.Handler, error) {
		req, _ := http.NewRequest("GET", "/", nil)
		req.Host = hostname
		resolved, err := getConfigByHostname(context.Background(), req, hostname, 0, m, cfg)
		if err != nil {
			return nil, fmt.Errorf("failed to resolve origin %q: %w", hostname, err)
		}
		return resolved, nil
	}
	// EmbeddedConfigLoader loads an inline origin config.
...
func saveFailsafeSnapshot(hostname string, cfg *config.Config, raw []byte) {
	if cfg == nil || cfg.Disabled || hostname == "" || len(raw) == 0 {
		return
	}
	failsafeSnapshots.save(hostname, cfg.WorkspaceID, cfg.Version, cfg.Revision, raw, cfg.FailsafeOrigin)
}

func loadFailsafeSnapshotConfig(ctx context.Context, hostname string, parent *config.Config, m manager.Manager, reason string, detail string) (*config.Config, error) {
	snapshot, ok := failsafeSnapshots.load(hostname)
	if !ok {
...
func loadExplicitFailsafeOriginConfig(ctx context.Context, hostname string, parent *config.Config, m manager.Manager) (*config.Config, error) {
	snapshot, ok := failsafeSnapshots.load(hostname)
	...
	cfg, err := resolveFailsafeOrigin(ctx, snapshot.FailsafeOrigin, parent, m)
	if err != nil {
		return nil, err
	}
	cfg.ConfigMode = configModeFailsafe
...
func resolveFailsafeOrigin(ctx context.Context, failsafeOrigin *config.FailsafeOrigin, parent *config.Config, m manager.Manager) (*config.Config, error) {
	if failsafeOrigin == nil {
		return nil, fmt.Errorf("failsafe origin is nil")
	}
	if failsafeOrigin.HasEmbeddedOrigin() {
		cfg, err := loadEmbeddedConfig(failsafeOrigin.Origin, parent, m)
		if err != nil {
			return nil, err
		}
		return cfg, nil
	...
	cfg, err := getConfigByHostname(ctx, nilRequestForHostname(failsafeOrigin.Hostname), failsafeOrigin.Hostname, 0, m, parent)
	if err != nil {
		return nil, err
	}
	return cfg, nil
...
func setupVaultManager(cfg *config.Config, m manager.Manager) {
	...
	localProvider := config.NewLocalVaultProvider(localCache)
	vm, err := config.NewVaultManager(localProvider)
	if err != nil {
	...
	mergedVaults := config.MergeVaults(config.GetServerVaults(), cfg.Vaults)
	...
	cfg.SecretsMap = nil
	if err := config.ProcessSecretFields(cfg, nil, decryptor, vm); err != nil {
...
// DrainWorkspace marks a workspace as draining. New requests will be rejected with 503.
func DrainWorkspace(workspaceID string) {
	drainedWorkspaces.Store(workspaceID, true)
	slog.Info("workspace marked as draining", "workspace_id", workspaceID)
}

// UndrainWorkspace removes the drain mark from a workspace.
func UndrainWorkspace(workspaceID string) {
	drainedWorkspaces.Delete(workspaceID)
	slog.Info("workspace drain removed", "workspace_id", workspaceID)
}

// IsWorkspaceDraining returns whether a workspace is currently being drained.
func IsWorkspaceDraining(workspaceID string) bool {
	_, draining := drainedWorkspaces.Load(workspaceID)
	return draining
}
