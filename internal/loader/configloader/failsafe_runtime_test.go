...
func TestGetConfigByHostname_UsesFailsafeSnapshotOnStorageError(t *testing.T) {
	...
	defaultLoader.cache.Delete("example.com")
...
func TestGetConfigByHostname_UsesFailsafeSnapshotOnInvalidConfig(t *testing.T) {
	...
	if _, err := defaultLoader.getConfigByHostname(context.Background(), req, "example.com", 0, mgr, nil); err != nil {
		t.Fatalf("initial load error = %v", err)
	}
...
func TestFailsafeSnapshot_PersistsToDisk(t *testing.T) {
	...
	originalStore := defaultLoader.failsafeSnapshots
	defaultLoader.failsafeSnapshots = newFailsafeSnapshotStore()
	defer func() {
		defaultLoader.failsafeSnapshots = originalStore
		defaultLoader.failsafeSnapshots.resetForTests()
	}()

	payload := createConfigJSON("example.com", "config-1", false, nil)
	defaultLoader.failsafeSnapshots.save("example.com", "ws-1", "1.0.0", "rev-1", payload, nil)

	defaultLoader.failsafeSnapshots = newFailsafeSnapshotStore()
	snapshot, ok := defaultLoader.failsafeSnapshots.load("example.com")
	if !ok {
...
func TestGetConfigByHostname_UsesExplicitFailsafeHostname(t *testing.T) {
	...
	defaultLoader.failsafeSnapshots.mu.Lock()
	defaultLoader.failsafeSnapshots.byHost["example.com"].Payload = nil
	defaultLoader.failsafeSnapshots.mu.Unlock()
	defaultLoader.cache.Delete("example.com")
	stor.getErrors = map[string]error{"example.com": context.DeadlineExceeded}
