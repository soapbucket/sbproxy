...
// TestClassifierTransformSidecarClassifyConfigParsing tests sidecar_classify config parsing.
func TestClassifierTransformSidecarClassifyConfigParsing(t *testing.T) {
	t.Run("full sidecar_classify config loads", func(t *testing.T) {
		data := json.RawMessage(`{
			"type": "sidecar_classify",
			"top_k": 3,
			"header_name": "X-Content-Class",
			"score_header": "X-Class-Score",
			"labels": [
				{"name": "billing", "weight": 1.2, "patterns": ["(?i)\\b(invoice|billing)\\b"]},
				{"name": "support", "weight": 1.0, "patterns": ["(?i)\\b(error|bug)\\b"]}
			],
			"classification": {
		...
		tc, err := config.LoadTransformConfig(data)
		if err != nil {
			t.Fatalf("failed to load sidecar_classify transform config: %v", err)
		}
		if tc.GetType() != "sidecar_classify" {
			t.Fatalf("expected type 'sidecar_classify', got %q", tc.GetType())
		}
	})

	t.Run("minimal sidecar_classify config loads with defaults", func(t *testing.T) {
		data := json.RawMessage(`{
			"type": "sidecar_classify",
			"labels": [
				{"name": "test", "patterns": ["hello"]}
			]
		}`)

		tc, err := config.LoadTransformConfig(data)
		if err != nil {
			t.Fatalf("failed to load minimal sidecar_classify config: %v", err)
		}
		if tc.GetType() != "sidecar_classify" {
			t.Fatalf("expected type 'sidecar_classify', got %q", tc.GetType())
		}
	...
	t.Run("unknown transform type fails", func(t *testing.T) {
