...
Set to "none" to disable request logging entirely.
...
SB_DISABLE_HOST_FILTER env var.`)
	...
	// --- sb-flags control ---
	viper.SetDefault(disableSbFlagsKey, false)
	if err := viper.BindEnv(disableSbFlagsKey, "SB_DISABLE_SB_FLAGS"); err != nil {
		log.Fatalf("failed to bind disable_sb_flags env: %v", err)
	}
	cmd.Flags().BoolVar(&cfg.DisableSbFlags, disableSbFlagsFlag, viper.GetBool(disableSbFlagsKey),
		`Disable X-Sb-Flags header and _sb.* query
parameter processing. When disabled, clients
cannot enable debug mode, bypass caches, or
control tracing via request headers. Use in
production for tighter control.
This flag can be set using
SB_DISABLE_SB_FLAGS env var.`)
	if err := viper.BindPFlag(disableSbFlagsKey, cmd.Flags().Lookup(disableSbFlagsFlag)); err != nil {
		log.Fatalf("failed to bind %s flag: %v", disableSbFlagsFlag, err)
	}
}
