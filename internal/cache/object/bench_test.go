...
// --- BenchmarkObjectCacheConcurrent ---

// BenchmarkObjectCacheConcurrent measures the sharded lock performance under
// high contention with a mix of Get, Put, Delete, and Increment operations
// across many goroutines. This benchmark validates that the 32-shard design
// reduces lock contention compared to a single-mutex approach.
func BenchmarkObjectCacheConcurrent(b *testing.B) {
	goroutines := []int{4, 16, 64}
	cacheSize := 10000
		...
		b.Run(fmt.Sprintf("goroutines=%d", numG), func(b *testing.B) {
			...
			b.RunParallel(func(pb *testing.PB) {
				...
				for pb.Next() {
					...
					case 2:
						cache.Delete(key)
						cache.Put(key, "re-inserted")
					case 3:
						cache.Increment(fmt.Sprintf("counter-%d", i%100), 1)
					}
					i++
				}
			})
