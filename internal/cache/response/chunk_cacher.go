...
func serializeChunk(chunk *CachedChunk) []byte {
	...
	// Write status
	writer.WriteByte(byte(chunk.Status >> 8))
	writer.WriteByte(byte(chunk.Status))
	...
	// Write headers count
	writer.WriteByte(byte(len(chunk.Headers)))
	for key, values := range chunk.Headers {
		writer.WriteString(key + "\n")
		writer.WriteByte(byte(len(values)))
	...
	writer.WriteByte(byte(bodyLen >> 24))
	writer.WriteByte(byte(bodyLen >> 16))
	writer.WriteByte(byte(bodyLen >> 8))
	writer.WriteByte(byte(bodyLen))
