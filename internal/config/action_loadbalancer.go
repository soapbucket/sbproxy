...
// selectLeastConnections selects the target with the least active connections
func (lb *loadBalancerTransport) selectLeastConnections() int {
	if len(lb.targets) == 0 {
		return -1
	}

	minConnections := atomic.LoadInt64(&lb.targets[0].activeConnections)
	minIndex := 0

	for i := 1; i < len(lb.targets); i++ {
		connections := atomic.LoadInt64(&lb.targets[i].activeConnections)
	...
	slog.Debug("selected target by least connections", "target_index", minIndex, "connections", minConnections)
	return minIndex
}

// selectWeightedRandom selects a target using weighted random selection
func (lb *loadBalancerTransport) selectWeightedRandom() int {
	if len(lb.targets) == 0 {
		return -1
	}
	...
	if totalWeight <= 0 {
		...
		lb.mu.Lock()
		idx := lb.random.Intn(len(lb.targets))
		lb.mu.Unlock()
		return idx
	}
...
// selectWeightedRoundRobin selects targets in round-robin order, with each target
// receiving a number of consecutive requests proportional to its weight.
// For example, with weights [3, 1], target 0 gets 3 requests, then target 1 gets 1.
func (lb *loadBalancerTransport) selectWeightedRoundRobin() int {
	if len(lb.targets) == 0 {
		return -1
	}
	...
	if counter <= 0 {
		idx = (idx + 1) % int64(len(lb.targets))
