...
type AIProxyActionConfig struct {
	...
	Cache *cache.SemanticCacheConfig `json:"cache,omitempty"`
	...
	// RAGProvider configures the external RAG provider backend.
	// The provider's Config map may contain secret values (API keys, tokens)
	// that are resolved through the secrets system (vault, encrypted, or template references).
	RAGProvider *rag.ProviderConfig `json:"rag_provider,omitempty"`
...
func (a *AIProxyAction) Init(cfg *Config) error {
	...
	if a.Budget != nil {
		...
		handlerCfg.Budget = ai.NewBudgetEnforcer(a.Budget, store)
	}

	if a.Cache != nil && a.Cache.Enabled {
		maxEntries := a.Cache.MaxEntries
		if maxEntries <= 0 {
			maxEntries = 10000
		}
		var vectorStore cache.VectorStore
		switch a.Cache.Store {
		case "", "memory":
			vectorStore = cache.NewMemoryVectorStore(maxEntries)
		case "redis":
			if cfg.l3Cache == nil {
				return fmt.Errorf("ai_proxy semantic cache store=redis requires L3 cache configuration")
			}
			vectorStore = cache.NewRedisVectorStore(cfg.l3Cache, maxEntries)
		default:
			return fmt.Errorf("ai_proxy semantic cache store %q is not supported", a.Cache.Store)
		}

		// Create embed function from a provider that supports embeddings
		var embedFn cache.EmbedFunc
		if a.Cache.EmbeddingProvider == "local" {
			mc := classifier.Global()
			embedFn = cache.NewLocalEmbedFunc(mc)
		} else {
			for _, pcfg := range a.Providers {
				if !pcfg.IsEnabled() {
					continue
				}
				// If embedding_provider is specified, only use that one
				if a.Cache.EmbeddingProvider != "" && pcfg.Name != a.Cache.EmbeddingProvider {
					continue
				}
				p, perr := ai.NewProvider(pcfg, httpClient)
				if perr != nil {
					continue
				}
				if p.SupportsEmbeddings() {
					embedModel := a.Cache.EmbeddingModel
					embedFn = cache.NewProviderEmbedFunc(&providerEmbedAdapter{provider: p, config: pcfg}, embedModel)
					break
				}
			}
		}
		...
		if embedFn != nil {
			handlerCfg.Cache = cache.NewSemanticCache(a.Cache, vectorStore, embedFn)
		}
	}
	...
	if a.Memory != nil && a.Memory.Enabled {
		chCfg := GetGlobalClickHouseConfig()
		if chCfg != nil {
			memWriter, merr := memory.NewWriter(logging.ClickHouseWriterConfig{
				Host:     chCfg.Host,
				Database: chCfg.Database,
			}, a.Memory)
			if merr != nil {
				return fmt.Errorf("failed to create memory writer: %w", merr)
			}
			handlerCfg.Memory = memWriter
		}
	}

	// Initialize virtual key management
	if a.VirtualKeys != nil && a.VirtualKeys.Enabled {
		switch a.VirtualKeys.Store {
		case "file", "":
			if a.VirtualKeys.FilePath == "" {
				return fmt.Errorf("ai_proxy virtual_keys: file_path is required when store=file")
			}
			keyStore, kerr := keys.NewFileStore(a.VirtualKeys.FilePath)
			if kerr != nil {
				return fmt.Errorf("failed to load virtual keys file: %w", kerr)
			}
			go keyStore.WatchFile(context.Background())
			a.virtualKeyStore = keyStore
		default:
			return fmt.Errorf("ai_proxy virtual_keys store %q is not supported", a.VirtualKeys.Store)
		}

		// Use Redis-backed usage tracker if L3 cache is available, otherwise in-memory
		if cfg.l3Cache != nil {
			handlerCfg.VirtualKeyUsage = keys.NewRedisUsageTracker(cfg.l3Cache)
		} else {
			handlerCfg.VirtualKeyUsage = keys.NewUsageTracker()
		}
	}

	// Initialize RAG pipeline
	if a.RAG != nil && a.RAG.Enabled {
		var ragRetriever ai.RAGRetriever

		if a.RAGProvider != nil && a.RAGProvider.Enabled {
			// External managed RAG provider (Pinecone, Vectara, etc.)
			registry := rag.DefaultRegistry()
			provider, rerr := registry.Create(*a.RAGProvider)
			if rerr != nil {
				return fmt.Errorf("failed to create RAG provider: %w", rerr)
			}
			ragRetriever = &ragProviderAdapter{provider: provider}
		} else {
			// Local vector store RAG
			storeType := a.RAG.ResolvedVectorDB()
			var vectorStore cache.VectorStore
			switch storeType {
			case "", "memory":
				vectorStore = cache.NewMemoryVectorStore(10000)
			case "redis":
				if cfg.l3Cache == nil {
					return fmt.Errorf("ai_proxy RAG store=redis requires L3 cache configuration")
				}
				vectorStore = cache.NewRedisVectorStore(cfg.l3Cache, 10000)
			default:
				return fmt.Errorf("ai_proxy RAG vector_db %q is not supported", storeType)
			}

			if a.RAG.EmbeddingProvider == "local" {
				mc := classifier.Global()
				ragRetriever = ai.NewLocalRAGRetriever(vectorStore, mc)
			} else {
				ragRetriever = ai.NewDefaultRAGRetriever(vectorStore)
			}

			// Load documents from file if configured
			if a.RAG.DocumentsFile != "" {
				if lerr := loadRAGDocuments(ragRetriever, a.RAG.DocumentsFile); lerr != nil {
					return fmt.Errorf("failed to load RAG documents: %w", lerr)
				}
			}
		}

		handlerCfg.RAG = ai.NewRAGPipeline(ragRetriever, a.RAG)
	}
...
// providerEmbedAdapter bridges ai.Provider to cache.ProviderEmbedder for semantic cache embeddings.
type providerEmbedAdapter struct {
	provider ai.Provider
	config   *ai.ProviderConfig
}

// Embed performs the embed operation on the providerEmbedAdapter.
func (a *providerEmbedAdapter) Embed(ctx context.Context, text, model string) ([]float32, error) {
	resp, err := a.provider.Embeddings(ctx, &ai.EmbeddingRequest{
		Input: text,
		Model: model,
	}, a.config)
	if err != nil {
