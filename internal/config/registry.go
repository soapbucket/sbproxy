...
// This provides backward compatibility with the init() registration pattern.
func DefaultRegistry() *Registry {
	r := NewRegistry()
	// Copy from legacy init()-populated maps
	for k, v := range loaderFns {
		r.actions[k] = v
	}
	for k, v := range authLoaderFuns {
		r.auths[k] = v
	}
	for k, v := range policyLoaderFns {
		r.policies[k] = v
	}
	for k, v := range transformLoaderFns {
		r.transforms[k] = v
	}
	return r
}
