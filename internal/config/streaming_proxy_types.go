...
// CompressionConfig controls proxy-level response compression (RFC 9110 Section 8.4)
...
// CORSConfig controls Cross-Origin Resource Sharing headers (RFC 6454, Fetch Standard)
...
// GetProxyHeaders returns ProxyHeaders or defaults if nil
func (c *Config) GetProxyHeaders() *ProxyHeaderConfig {
	if c.ProxyHeaders == nil {
		return DefaultProxyHeaders
	}
	return c.ProxyHeaders
}

// GetProxyProtocol returns ProxyProtocol or defaults if nil
func (c *Config) GetProxyProtocol() *ProxyProtocolConfig {
...
// GetStreamingProxyConfig returns StreamingProxyConfig or defaults if nil
func (c *Config) GetStreamingProxyConfig() *StreamingProxyConfig {
	if c.StreamingProxyConfig == nil {
		return DefaultStreamingConfig
	}
	return c.StreamingProxyConfig
}
