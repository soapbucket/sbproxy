...
// TestWAFBlocking tests that WAF actually blocks requests when rules match
func TestWAFBlocking(t *testing.T) {
	...
	// Create a test handler that would be called if request passes
	backendHandler := http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(http.StatusOK)
		w.Write([]byte("OK"))
	})

	// Apply policies to create the middleware chain
	...
	t.Run("Normal request passes", func(t *testing.T) {
		req := httptest.NewRequest("GET", "http://waf.test/", nil)
	...
	// Test 2: SQL injection in query should be blocked
	t.Run("SQL injection blocked", func(t *testing.T) {
		req := httptest.NewRequest("GET", "http://waf.test/?id=1%27%20OR%20%271%27%3D%271", nil)
		w := httptest.NewRecorder()
		handler.ServeHTTP(w, req)
		...
		// Accept either "blocked", "Access Denied", or "Security policy violation" as valid blocking messages
		body := w.Body.String()
		if !strings.Contains(strings.ToLower(body), "denied") &&
		   !strings.Contains(strings.ToLower(body), "blocked") &&
		   !strings.Contains(strings.ToLower(body), "security") {
			t.Errorf("Expected error message about blocking, got: %s", body)
		}
	})

	// Test 3: Path traversal should be blocked (if rule exists)
	t.Run("Path traversal blocked", func(t *testing.T) {
		req := httptest.NewRequest("GET", "http://waf.test/../../../etc/passwd", nil)
		w := httptest.NewRecorder()
		handler.ServeHTTP(w, req)

		// This should pass if no path traversal rule, or be blocked if rule exists
...
// TestWAFMultipleRules tests WAF with multiple rules
func TestWAFMultipleRules(t *testing.T) {
	configJSON := `{
		"id": "waf-multiple-test",
	...
	t.Run("SQL injection blocked", func(t *testing.T) {
		req := httptest.NewRequest("GET", "http://waf-multiple.test/?id=1%27%20OR%20%271%27%3D%271", nil)
		w := httptest.NewRecorder()
		handler.ServeHTTP(w, req)

		if w.Code != http.StatusForbidden {
			t.Errorf("Expected status 403 for SQL injection, got %d", w.Code)
		}
	})
