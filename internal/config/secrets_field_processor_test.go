...
func TestProcessSecretField_LoadedSecret(t *testing.T) {
	// Create a secrets manager with a secret
	secretsManager := NewSecretsManager()
	secretsManager.allSecrets = map[string]string{
		"secret": "loaded-secret-value",
	}

	decryptor, _ := crypto.NewDecryptorFromEnv()

	// Test that loaded secret takes precedence
...
func TestProcessSecretField_EncryptedString(t *testing.T) {
	// Set up environment for local encryption
	...
	// Test that encrypted string is decrypted
...
func TestProcessSecretField_PlainText(t *testing.T) {
	// Test that plain text logs warning but succeeds
...
func TestProcessSecretField_Priority(t *testing.T) {
	// Test priority: loaded secret > encrypted > plain text
	key, err := crypto.GenerateKey()
	if err != nil {
		t.Fatalf("failed to generate key: %v", err)
	}
	...
	// Even if encrypted value is provided, loaded secret should win
...
func TestProcessSecretFields_StringSlice(t *testing.T) {
	...
	// Each element should be processed individually
	// Since we're using the same key for all, they'll all get the loaded secret
	if len(cfg.APIKeys) != 2 {
		t.Errorf("expected 2 keys, got %d", len(cfg.APIKeys))
	}
	// Note: In real usage, you'd use different keys or process differently
	// This test verifies the slice processing works
}
...
func TestProcessSecretFields_RealConfig(t *testing.T) {
	// Test with a real config structure
	// This test verifies that LoadWithContext works correctly without secrets configured
	// The secret field should remain plain text (with a warning logged)
	configJSON := `{
		"id": "test",
	...
	cfg, err := LoadWithContext(context.Background(), []byte(configJSON))
	if err != nil {
	...
	if cfg.ID != "test" {
		t.Errorf("expected config ID 'test', got %q", cfg.ID)
	}
	if cfg.Hostname != "example.com" {
		t.Errorf("expected hostname 'example.com', got %q", cfg.Hostname)
	}

	// The secret should have been processed
	// Since we don't have secrets in the config, it should remain plain text (with warning)
	// ProcessSecretFields will log a warning but allow plain text secrets for testing/development
}

func TestProcessSecretField_VaultPrefix(t *testing.T) {
	// Create a vault manager with a cached secret
...
func TestProcessSecretField_VaultPrefix_NoVaultManager(t *testing.T) {
	// When no vault manager is configured, vault: prefix should fall through to plain text
	result, err := ProcessSecretField("vault:some-key", "api_key", nil, nil)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	// Falls through to plain text (step 6) since no vault manager is available
...
func TestProcessSecretFields_ArrayOfStructs(t *testing.T) {
	...
	// All passwords should be replaced with loaded secret
...
func TestProcessSecretFields_AuditLog_KeyType(t *testing.T) {
	...
	// Build a map of field -> key_type from the audit records
...
func TestVaultManager_ResolveAll_AuditLog(t *testing.T) {
	handler := newCaptureHandler()
	oldLogger := slog.Default()
	slog.SetDefault(slog.New(handler))
	defer slog.SetDefault(oldLogger)
	...
	mock := NewMockVaultProvider(VaultTypeLocal)
	mock.SetSecret("/gateway/token", "tok-123")
	...
	records := handler.findRecords("secret resolved")
	if len(records) == 0 {
		t.Fatal("expected 'secret resolved' audit log entry from ResolveAll, got none")
	}

	r := records[0]
	if got := recordAttr(r, "field"); got != "GW_TOKEN" {
		t.Errorf("audit log field = %q, want %q", got, "GW_TOKEN")
	}
	if got := recordAttr(r, "source"); got != "vault" {
		t.Errorf("audit log source = %q, want %q", got, "vault")
	}
	if got := recordAttr(r, "vault_name"); got != "system" {
		t.Errorf("audit log vault_name = %q, want %q", got, "system")
	}
}
