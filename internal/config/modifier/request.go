...
func applyHeaderModifications(header http.Header, mods *HeaderModifications) {
	applyHeaderModificationsWithRequest(header, mods, nil)
}
...
func resolveTemplateVariablesLegacy_REMOVED(value string, req *http.Request) string {
	// This function has been removed - use template.Resolve() instead
	// Keeping signature for reference only
	return value
}

func removedLegacyCode() {
	// The following regex-based implementation has been replaced by Mustache:
	//
	// templateVarPattern := regexp.Compile(`\{\{(request|original|config|...)\.([^}]+)\}\}`)
	// return templateVarPattern.ReplaceAllStringFunc(value, func(match string) string {
	//     // Extract and resolve variable
	//     // ... 200+ lines of nested switch statements ...
	// })
	//
	// Benefits of Mustache replacement:
	// - Simpler code (10 lines vs 200+)
	// - Sections for conditionals and iteration
	// - Better error messages
	// - Consistent across all components
	// - Cached compilation for performance
}

// Note: ~200 lines of legacy regex-based template code was removed here
// All template resolution now uses: template.Resolve(value, req)
// This provides the same functionality with Mustache syntax:
// - Sections: {{#condition}}...{{/condition}}
// - Inverted sections: {{^falsy}}default{{/falsy}}
// - Iteration: {{#items}}{{field}}{{/items}}
// - Same variables: request, original, config, request_data, session, auth, secrets

// getNestedValue and helper functions below are still used by other parts of the modifier system
// (not just templates, also for CEL/Lua data access)

// End of legacy code removal marker

// Helper functions below this line are still actively used

// getNestedValue retrieves a nested value from a map using dot-separated path
// Example: getNestedValue(map{"config_data": map{"api_version": "v2"}}, "config_data.api_version") returns "v2"
// Handles both native Go types (map[string]any) and CEL ref.Val types (map[ref.Val]ref.Val)
func getNestedValue(m map[string]any, path string) string {
	...
	for i, part := range parts {
		...
		if val, ok := current[part]; ok {
			nextMap := convertToMap(val)
			if nextMap != nil {
				current = nextMap
			} else {
				// Not a map, can't continue
				return ""
			}
		} else {
			// Path not found
...
// convertToMap converts a value to map[string]any
// CEL/Lua packages now return native types, so this is simpler
func convertToMap(val interface{}) map[string]any {
	// Try direct conversion first
	if m, ok := val.(map[string]any); ok {
		return m
	}

	// Try map[string]interface{} (from CEL package)
...
// convertValueToString converts a value to string
// CEL/Lua packages now return native types, so this is simpler
...
// maskSecret masks a secret value for logging (shows first 4 and last 4 chars)
...
// getSecretKeys returns a slice of secret keys for logging
