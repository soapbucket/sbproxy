...
func createHMACSignature(secret string, data string) string {
	h := hmac.New(sha256.New, []byte(secret))
	h.Write([]byte(data))
...
func (m *mockHTTPClient) JSONPost(ctx context.Context, url string, body any, headers http.Header) (*http.Response, error) {
	if m.err != nil {
		return nil, m.err
	}
...
func createMockResponse(statusCode int, body map[string]any, headers map[string]string) *http.Response {
