...
func TestEventHelpers_EmitSecurityAndUpstreamEvents(t *testing.T) {
	msg := &testMessenger{}
	...
	emitSecurityAuthFailure(ctx, cfg, req, "basic", "invalid_credentials")
	emitSecurityRateLimited(ctx, cfg, req, "per_minute", 10, "1m")
	emitSecurityPIIDetected(ctx, cfg, req, []string{"email", "ssn"})
	emitUpstreamTimeout(ctx, cfg, req, "http://upstream.internal", 30)
	emitUpstream5xx(ctx, cfg, req, "http://upstream.internal", 503, 12)
	emitTypedCircuitEvent(cfg, "upstream.circuit_opened", ievents.SeverityWarning, "http://upstream.internal", 5, 30, 0)
	emitTypedCircuitEvent(cfg, "upstream.circuit_closed", ievents.SeverityInfo, "http://upstream.internal", 0, 0, 3)

	channel := "test:events:ws-1"
	if got := len(msg.messages[channel]); got != 7 {
		t.Fatalf("expected 7 emitted events, got %d", got)
	}

	typeNames := make([]string, 0, 7)
	...
	expected := map[string]bool{
		"security.auth_failure":   false,
		"security.rate_limited":   false,
		"security.pii_detected":   false,
		"upstream.timeout":        false,
		"upstream.5xx":            false,
		"upstream.circuit_opened": false,
		"upstream.circuit_closed": false,
	}
