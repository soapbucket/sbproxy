...
/*
func TestRequestRule_Match_MaxMind(t *testing.T) {
	tests := []struct {
		name     string
		rule     RequestRule
		req      *http.Request
		...
		{
			name:     "MaxMind - Country code matches",
			rule:     RequestRule{MaxMind: &MaxMindRule{CountryCodes: []string{"US"}}},
			req:      mustCreateRequest("GET", "https://example.com/path", t),
			location: &libmaxmind.Result{CountryCode: "US"},
			expected: true,
		},
		{
			name:     "MaxMind - Country code does not match",
			rule:     RequestRule{MaxMind: &MaxMindRule{CountryCodes: []string{"US"}}},
			req:      mustCreateRequest("GET", "https://example.com/path", t),
			location: &libmaxmind.Result{CountryCode: "GB"},
			expected: false,
		},
		{
			name:     "MaxMind - Multiple country codes, one matches",
			rule:     RequestRule{MaxMind: &MaxMindRule{CountryCodes: []string{"US", "GB", "CA"}}},
			req:      mustCreateRequest("GET", "https://example.com/path", t),
			location: &libmaxmind.Result{CountryCode: "GB"},
			expected: true,
		},
		{
			name:     "MaxMind - Country code case-insensitive",
			rule:     RequestRule{MaxMind: &MaxMindRule{CountryCodes: []string{"us"}}},
			req:      mustCreateRequest("GET", "https://example.com/path", t),
			location: &libmaxmind.Result{CountryCode: "US"},
			expected: true,
		...
		{
			name:     "MaxMind - Country name matches",
			rule:     RequestRule{MaxMind: &MaxMindRule{Countries: []string{"United States"}}},
			req:      mustCreateRequest("GET", "https://example.com/path", t),
			location: &libmaxmind.Result{Country: "United States"},
			expected: true,
		},
		{
			name:     "MaxMind - Continent code matches",
			rule:     RequestRule{MaxMind: &MaxMindRule{ContinentCodes: []string{"NA"}}},
			req:      mustCreateRequest("GET", "https://example.com/path", t),
			location: &libmaxmind.Result{ContinentCode: "NA"},
			expected: true,
		},
		{
			name:     "MaxMind - ASN matches",
			rule:     RequestRule{MaxMind: &MaxMindRule{ASNs: []string{"AS7018"}}},
			req:      mustCreateRequest("GET", "https://example.com/path", t),
			location: &libmaxmind.Result{ASN: "AS7018"},
			expected: true,
		},
		{
			name:     "MaxMind - AS name does not match (exact required)",
			rule:     RequestRule{MaxMind: &MaxMindRule{ASNames: []string{"AT&T"}}},
			req:      mustCreateRequest("GET", "https://example.com/path", t),
			location: &libmaxmind.Result{ASName: "AT&T Enterprises, LLC"},
			expected: false,
		},
		{
			name:     "MaxMind - AS name matches (case-insensitive)",
			rule:     RequestRule{MaxMind: &MaxMindRule{ASNames: []string{"at&t enterprises, llc"}}},
			req:      mustCreateRequest("GET", "https://example.com/path", t),
			location: &libmaxmind.Result{ASName: "AT&T Enterprises, LLC"},
			expected: true,
		},
		{
			name:     "MaxMind - Multiple fields must all match",
			rule:     RequestRule{MaxMind: &MaxMindRule{CountryCodes: []string{"US"}, ContinentCodes: []string{"NA"}}},
			req:      mustCreateRequest("GET", "https://example.com/path", t),
			location: &libmaxmind.Result{CountryCode: "US", ContinentCode: "NA"},
			expected: true,
		},
		{
			name:     "MaxMind - Multiple fields, one does not match",
			rule:     RequestRule{MaxMind: &MaxMindRule{CountryCodes: []string{"US"}, ContinentCodes: []string{"EU"}}},
			req:      mustCreateRequest("GET", "https://example.com/path", t),
			location: &libmaxmind.Result{CountryCode: "US", ContinentCode: "NA"},
			expected: false,
		},
		{
			name:     "MaxMind - No location data",
			rule:     RequestRule{MaxMind: &MaxMindRule{CountryCodes: []string{"US"}}},
			req:      mustCreateRequest("GET", "https://example.com/path", t),
			location: nil,
			expected: false,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if tt.location != nil {
				maxmind.Put(tt.req, tt.location)
			}
			result := tt.rule.Match(tt.req)
			if result != tt.expected {
				t.Errorf("Expected Match() = %v, got %v", tt.expected, result)
...
// Temporarily disabled due to refactoring
/*
func maxMindRulesEqual(a, b *MaxMindRule) bool {
	if a == nil && b == nil {
		return true
	}
	if a == nil || b == nil {
