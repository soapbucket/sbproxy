...
func (g *GraphQLResponseConditions) match(resp *http.Response) bool {
	...
	} else if !g.HasErrors && len(gqlResp.Errors) > 0 {
		// If HasErrors is false (default), we don't want errors
		// But we only check this if HasErrors was explicitly set
		// For now, we'll only check if HasErrors is true
	}
