...
func (c *Config) reloadOnLoadIfNeeded(ctx context.Context) {
	...
	if c.Params != nil && !c.onLoadLastExecuted.IsZero() && !c.onLoadReloading {
		...
		slog.Debug("on_load cache expired, triggering background reload (serving stale)",
			"origin_id", c.ID,
			"hostname", c.Hostname,
			"time_since_last_exec", time.Since(c.onLoadLastExecuted),
...
// shouldReloadOnLoad checks if on_load cache has expired without acquiring the mutex.
func (c *Config) shouldReloadOnLoad(minCacheDuration time.Duration) bool {
	if c.onLoadLastExecuted.IsZero() {
		return true
	}
	return time.Since(c.onLoadLastExecuted) >= minCacheDuration
}
