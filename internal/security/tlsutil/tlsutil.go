...
// Reload re-reads all certificate and key files from disk. Each key pair is
// reloaded by calling tls.LoadX509KeyPair to verify the files are valid before
// accepting them. If any pair fails to load, the error is returned and the
// previously loaded pairs remain unchanged.
func (cm *CertManager) Reload() error {
	// Verify all key pairs can be loaded before accepting any changes.
	for id, kp := range cm.keyPairs {
		if _, err := tls.LoadX509KeyPair(kp.Cert, kp.Key); err != nil {
			return fmt.Errorf("reload key pair %q: %w", id, err)
		}
	}
	// GetCertificateFunc already calls tls.LoadX509KeyPair on each handshake,
	// so reloading is effectively a validation pass. If certificates are cached
	// in the future, this is where the cache would be invalidated.
	slog.Info("tls certificates reloaded successfully",
		"pairs", len(cm.keyPairs))
	return nil
}
