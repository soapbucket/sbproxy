...
func NewCertificatePinner(config *CertificatePinningConfig, originName string) (*CertificatePinner, error) {
	if config == nil {
...
func (p *CertificatePinner) VerifyPeerCertificate(rawCerts [][]byte, verifiedChains [][]*x509.Certificate) error {
	...
	if p.config.PinExpiry != "" {
		...
		if err != nil {
			...
			slog.Error("certificate pin has expired",
				"origin", p.originName,
				"expiry", expiryTime)
			metric.CertPinVerification(p.originName, "expired")
			metric.CertPinVerificationError(p.originName, "expired")
			// Also record using the critical metric
			metric.CertPinFailure(p.originName, "expired")
	...
	if len(rawCerts) == 0 {
		slog.Error("no certificates provided for pinning verification",
			"origin", p.originName)
		metric.CertPinVerification(p.originName, "no_certs")
		metric.CertPinVerificationError(p.originName, "no_certs")
		// Also record using the critical metric
		metric.CertPinFailure(p.originName, "no_certificates")
		...
		for _, pin := range validPins {
			if spkiHashBase64 == pin {
				slog.Debug("certificate pin matched",
					...
					"pin", pin)
				metric.CertPinVerification(p.originName, "success")
				return nil
			}
		}
	...
	metric.CertPinVerification(p.originName, "mismatch")
	metric.CertPinVerificationError(p.originName, "pin_mismatch")
	// Also record using the critical metric
...
func ValidateConfig(config *CertificatePinningConfig) error {
	if config == nil {
		return nil
	}
