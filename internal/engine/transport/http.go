...
// CleanTransportCache removes all cached transports, forcing new ones to be
// created on next use. Call this during config reload when origins change
// substantially, or periodically to prevent unbounded growth.
// Existing in-flight requests using old transports are unaffected because
// they hold a reference to the transport via the handler chain.
func CleanTransportCache() {
