...
type HTTPCacheTransport struct {
	...
	store  cacher.Cacher
	config *CacheConfig
	sfg    singleflight.Group // deduplicates concurrent cache misses for the same key
}
...
func (h *HTTPCacheTransport) RoundTrip(req *http.Request) (*http.Response, error) {
	...
	// Use singleflight to deduplicate concurrent cache misses for the same key.
	// This prevents a thundering herd of identical backend requests when multiple
	// clients request the same uncached resource simultaneously.
	type sfResult struct {
		resp *http.Response
		err  error
	}

	v, err, _ := h.sfg.Do(cacheKey, func() (interface{}, error) {
		// Double-check: another goroutine in the same singleflight group may
		// have populated the cache while we were waiting.
		if cachedResp := h.getCachedResponse(req.Context(), req, cacheKey); cachedResp != nil {
			return &sfResult{resp: cachedResp}, nil
		}
