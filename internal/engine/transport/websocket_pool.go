...
type WebSocketConnectionPool struct {
	...
	origin string // Origin ID for metrics
	...
	stats struct {
		...
		totalAcquired   int64
		totalReleased   int64
		totalExhausted  int64 // Times pool was exhausted
	}
}
...
func NewWebSocketConnectionPoolWithDialer(target string, config *WebSocketPoolConfig, customDialer *websocket.Dialer) (*WebSocketConnectionPool, error) {
	return NewWebSocketConnectionPoolWithDialerAndOrigin(target, config, customDialer, "")
}

// NewWebSocketConnectionPoolWithDialerAndOrigin creates a new WebSocket connection pool with a custom dialer and origin
func NewWebSocketConnectionPoolWithDialerAndOrigin(target string, config *WebSocketPoolConfig, customDialer *websocket.Dialer, origin string) (*WebSocketConnectionPool, error) {
	if config == nil {
...
func (p *WebSocketConnectionPool) Acquire(ctx context.Context) (*PooledWebSocketConn, error) {
	...
	// Record metric
	metric.WebSocketPoolConnectionAcquired(p.origin, p.target)
	...
	if !canCreate {
		...
		metric.ConnectionPoolExhaustion(p.origin, "websocket")
		p.stats.mu.Lock()
		p.stats.totalExhausted++
		p.stats.mu.Unlock()
...
func (p *WebSocketConnectionPool) Release(conn *PooledWebSocketConn) {
	...
	// Record metric
	metric.WebSocketPoolConnectionReleased(p.origin, p.target)
	...
	case p.idle <- conn:
		...
		p.updatePoolMetrics() // Update metrics when connection is returned
	default:
...
func (p *WebSocketConnectionPool) createConnection(ctx context.Context) (*PooledWebSocketConn, error) {
	...
	// Record pool metrics
	metric.WebSocketPoolConnectionCreated(p.origin, p.target)
	p.updatePoolMetrics()
...
func (p *WebSocketConnectionPool) Close() error {
	...
	// Collect connections under the same write lock where closed is set.
	// This prevents a race where new connections could be added between
	// setting closed=true and iterating the map.
...
func (p *WebSocketConnectionPool) Stats() map[string]interface{} {
	...
	activeCount := currentSize - idleCount
	maxConnections := p.config.MaxConnections
	maxIdleConnections := p.config.MaxIdleConnections
	p.mu.RUnlock()

	// Calculate utilization percentages
	utilizationPercent := 0.0
	if maxConnections > 0 {
		utilizationPercent = float64(currentSize) / float64(maxConnections) * 100.0
	}

	idleUtilizationPercent := 0.0
	if maxIdleConnections > 0 {
		idleUtilizationPercent = float64(idleCount) / float64(maxIdleConnections) * 100.0
	}

	// Calculate reuse rate (connections reused vs created)
	reuseRate := 0.0
	if p.stats.totalCreated > 0 {
		reuseRate = float64(p.stats.totalAcquired-p.stats.totalCreated) / float64(p.stats.totalCreated) * 100.0
	}
...
// updatePoolMetrics updates all pool size metrics
func (p *WebSocketConnectionPool) updatePoolMetrics() {
	p.mu.RLock()
	currentSize := int64(len(p.connections))
	idleCount := int64(len(p.idle))
	activeCount := currentSize - idleCount
	p.mu.RUnlock()

	metric.WebSocketPoolSizeSet(p.origin, p.target, currentSize)
	metric.WebSocketPoolIdleSizeSet(p.origin, p.target, idleCount)
	metric.WebSocketPoolActiveSizeSet(p.origin, p.target, activeCount)
}
...
func (c *PooledWebSocketConn) Close() error {
	...
	// Record metric
	metric.WebSocketPoolConnectionClosed(c.pool.origin, c.pool.target)
	c.pool.updatePoolMetrics()
...
func (c *PooledWebSocketConn) Reconnect(ctx context.Context) error {
	...
	// Record metric
	metric.WebSocketPoolReconnect(c.pool.origin, c.pool.target)
