// detector.go inspects HTTP requests to identify and authenticate credentials.
package identity
...
func (d *CredentialDetector) Resolve(ctx context.Context, r *http.Request) (*Principal, error) {
	...
	// Store in cache if configured (cache is read-through via L3 connector wired by callers).
