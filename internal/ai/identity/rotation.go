// rotation.go implements API key rotation with configurable grace periods.
package identity
...
func (kr *KeyRotator) RevokeDeprecated(_ context.Context, oldKeyHash string) error {
	...
	// Invalidate cache if available (cache naturally expires via TTL; original credential unavailable for key lookup).
