...
func TestRESTConnector_Resolve_Success(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if req.CredentialType != "api_key" || req.Credential != "sk-test-123" {
	...
	perm, err := c.Resolve(context.Background(), "api_key", "sk-test-123")
	if err != nil {
...
func TestRESTConnector_Resolve_ServerError(t *testing.T) {
	...
	c := NewRESTConnector(server.URL, "", 5*time.Second)
	_, err := c.Resolve(context.Background(), "api_key", "sk-test-123")
	if err == nil {
