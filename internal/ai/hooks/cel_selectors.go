// cel_selectors.go implements CEL-based routing selectors for the AI gateway.
//
// Selectors let operators write CEL expressions that dynamically control
// routing decisions per-request. There are four selector types:
//
//   - model_selector: returns a string model name to override the request model
//   - provider_selector: returns a string provider name to prefer
//   - cache_bypass: returns a bool; true skips the semantic cache
//   - dynamic_rpm: returns an int to override the per-model RPM limit
//
// Expressions are compiled once at config load time via [NewCELSelector] and
// evaluated per-request via the Select*/ShouldBypass/DynamicRPM methods.
// The CEL context available to expressions is defined in [AIRequestCELContext].
package hooks
...
// selectorEnvOnce guards one-time initialization of the shared CEL environment.
// The environment is shared across all CEL selectors because they all use
// the same set of variables (request, headers, key, workspace, timestamp).
var (
