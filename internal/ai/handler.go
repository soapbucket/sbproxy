...
type HandlerConfig struct {
	...
	// Memory writes structured AI conversation data to storage.
	Memory MemoryWriter `json:"-"`
...
func (h *Handler) handleChatCompletion(w http.ResponseWriter, r *http.Request) {
	...
	if resp, ok := h.tryDegradedResponse(r.Context(), &req, semanticPromptText); ok {
		...
		if err != nil {
			slog.Error("failed to marshal degraded response", "error", err)
			WriteError(w, ErrInternal("failed to encode response"))
			return
		}
		if _, err := w.Write(respBytes); err != nil {
			slog.Error("failed to write degraded response", "error", err)
		}
		return
	}
...
func (h *Handler) handleNonStreamingCompletion(w http.ResponseWriter, r *http.Request, req *ChatCompletionRequest, entry providerEntry, semanticPromptText string) (*Usage, *ChatCompletionResponse, error) {
	...
	respBytes, err := json.Marshal(resp)
	if err != nil {
		slog.Error("failed to marshal completion response", "error", err)
		WriteError(w, ErrInternal("failed to encode response"))
		return nil, nil, err
	}
	if _, writeErr := w.Write(respBytes); writeErr != nil {
		slog.Error("failed to write completion response", "error", writeErr)
	}
...
func (h *Handler) handleStreamingCompletion(w http.ResponseWriter, r *http.Request, req *ChatCompletionRequest, entry providerEntry) (*Usage, *StreamAccumulator, time.Duration, time.Duration, error) {
	...
	for {
		...
		if err != nil {
			...
			_ = sw.WriteError(ErrInternal(err.Error()))
...
func (h *Handler) recordUsage(ctx context.Context, provider, model string, usage *Usage, streaming bool, latency, ttft, avgItl time.Duration) { //nolint:gocyclo
	...
	if rd.OriginalRequest != nil {
		if meta := ExtractMetadataFromHeaders(rd.OriginalRequest.GetHeaders()); len(meta) > 0 {
...
func (h *Handler) emitRequestCompleted(ctx context.Context, provider, model string, usage *Usage, latency, ttft time.Duration) {
	...
	// Record Prometheus metrics for AI traffic monitoring
	metric.AIRequestCompleted(
		provider, model,
		usage.PromptTokens, usage.CompletionTokens, usage.PromptTokensCached,
		latency.Seconds(), ttft.Seconds(),
	)
}
...
func (h *Handler) emitRequestFailed(ctx context.Context, model, provider string, writeErr error, httpStatus int, latencyMs int64, retries int) {
	...
	// Record Prometheus metrics for failed AI request
	metric.AIRequestFailed(provider, model, errorType)
}
...
func (h *Handler) emitProviderFallback(ctx context.Context, model, fromProvider, toProvider, reason string) {
	...
	// Record Prometheus metrics for provider fallback
	metric.AIFallback(fromProvider, toProvider, reason)
}

// messagesAsMapSlice converts typed Messages to map slices for extractPromptText.
...
func (h *Handler) semanticCachePromptText(msgs []Message) string {
	...
	return extractPromptText(h.messagesAsMapSlice(msgs))
}
...
func (h *Handler) handleGuardrailsCheck(w http.ResponseWriter, r *http.Request) {
	...
	if h.config.Guardrails == nil {
		...
		_ = json.NewEncoder(w).Encode(map[string]string{"error": "guardrails not configured"})
		return
	}
