// fallback_context.go implements automatic fallback to models with larger
// context windows.
//
// When a request exceeds a model's context window, FindLarger checks two
// sources in order:
//
//  1. Explicit configured mappings (e.g., "gpt-4" -> "gpt-4-turbo-128k")
//  2. Auto-generated candidates from the same provider in the registry
//
// Auto-generation picks the smallest sufficient window to minimize cost
// escalation. For example, if gpt-4 (8k) is too small, the fallback
// picks gpt-4-turbo (128k) rather than jumping to a more expensive model
// when gpt-4-32k would suffice.
package routing
