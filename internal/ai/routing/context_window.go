// context_window.go implements pre-call context window validation.
//
// The validator estimates input tokens and compares them (plus the requested
// output tokens) against the model's context window, reduced by a safety
// margin. This catches requests that would be rejected by the provider
// and gives the caller a chance to use a fallback model before making a
// network call.
//
// Token estimation is intentionally approximate (~4 chars per token) because
// exact tokenization requires model-specific tokenizers that add latency
// and binary dependencies. The safety margin compensates for underestimation.
package routing
