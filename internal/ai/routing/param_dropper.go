// param_dropper.go removes unsupported parameters from AI requests based
// on provider and model capabilities.
//
// Dropping is preferable to erroring because it enables cross-provider
// compatibility without requiring callers to maintain per-provider request
// templates. For example, a request with tools + vision sent to a
// text-only model will have image content stripped but still succeed with
// the text portions.
//
// The dropper handles: vision content (image_url parts), tools/tool_choice,
// response_format (structured output), thinking config, and reasoning_effort.
// Streaming is intentionally not dropped because changing the response
// format mid-request would break the caller's SSE parser.
package routing
