// Package callback executes HTTP callbacks (webhooks) to external services during
// request processing. Callbacks can be synchronous (blocking the request pipeline)
// or asynchronous (fire-and-forget via the Async flag).
//
// Key features:
//   - Singleflight coalescing prevents cache stampede on concurrent cache misses
//   - Circuit breaker protection per upstream URL
//   - Variable chaining: sequential callbacks can reference prior results in templates
//   - Negative caching: error responses are cached briefly to prevent retry storms
//   - Signature verification for webhook response authenticity (HMAC or RSA)
package callback
...
// When multiple requests trigger the same callback simultaneously, only one HTTP
// call is made and the result is shared with all waiters.
...
type Callback struct {
	...
	// Async controls callback vs webhook behavior:
	// false (default) = synchronous callback, blocks the request pipeline until complete.
	// true = asynchronous webhook, fires in a background goroutine without blocking.
	...
	// Transforms is a list of raw transform configs applied to the callback response
	// after Lua modification and before variable wrapping.
	// Each entry is a JSON object with at least a "type" field matching a registered transform.
	// Transforms operate on a synthetic http.Response wrapping the JSON body.
...
// This interface decouples the callback package from the config transform system,
// allowing the config package to compile transforms and inject them.
type TransformHandler interface {
	Apply(*http.Response) error
}

// TransformLoaderFn loads and compiles a transform from raw JSON config.
// Set via SetTransformLoader by the config package at init time.
type TransformLoaderFn func(json.RawMessage) (TransformHandler, error)

var transformLoader TransformLoaderFn

// SetTransformLoader registers the function used to compile transforms from JSON.
// Called by the config package to bridge the transform system into callbacks
// without creating a circular import.
func SetTransformLoader(fn TransformLoaderFn) {
	transformLoader = fn
}

// SetTransforms injects pre-compiled transform handlers into the callback.
// Called by the config package after loading transforms.
func (c *Callback) SetTransforms(handlers []TransformHandler) {
	c.transforms = handlers
}
...
func (c *Callback) UnmarshalJSON(data []byte) error {
	...
	*c = Callback(aux)
	...
	if c.CELExpr != "" {
		celMod, err := celext.CompileJSONModifier(c.CELExpr)
		if err != nil {
			return fmt.Errorf("callback: failed to compile cel_expr: %w", err)
		}
		c.celModifier = celMod
	}

	// Validate: LuaScript and CELExpr are mutually exclusive
	if c.LuaScript != "" && c.CELExpr != "" {
		return fmt.Errorf("callback: lua_script and cel_expr are mutually exclusive")
	}
	...
	// Compile transforms if a loader is registered and transforms are configured
	if len(c.Transforms) > 0 && transformLoader != nil {
		c.transforms = make([]TransformHandler, 0, len(c.Transforms))
		for i, rawTr := range c.Transforms {
			handler, err := transformLoader(rawTr)
			if err != nil {
				return fmt.Errorf("callback: failed to compile transform[%d]: %w", i, err)
			}
			c.transforms = append(c.transforms, handler)
		}
	}

	c.cacheKey = crypto.GetHashFromString(fmt.Sprintf("%s:%s:%v:%v:%s:%s:%s:%d:%s:%s:%v:%v", c.URL, c.Method, c.Headers, c.ExpectedStatusCodes, c.VariableName, c.LuaScript, c.CELExpr, c.CacheDuration.Duration, c.Body, c.ContentType, c.FormFields, c.SkipTLSVerifyHost))
...
func (c *Callback) Do(ctx context.Context, obj any) (map[string]any, error) {
	slog.Debug("executing callback", "url", c.URL, "headers", c.Headers, "method", c.Method, "expected_status_codes", c.ExpectedStatusCodes, "variable_name", c.VariableName, "lua_script", c.LuaScript, "cache_duration", c.CacheDuration)
...
func (c *Callback) executeCallback(ctx context.Context, obj any) (map[string]any, error) {
	...
	if len(data) > 0 && strings.EqualFold(respContentType, "application/json") {
		...
		// Apply Lua modifications if configured
		if c.luaScript != nil {
		...
		// Apply CEL modifications if configured
		if c.celModifier != nil {
			if resultMap, ok := jsonResult.(map[string]any); ok {
				jsonResult, err = c.celModifier.ModifyJSON(resultMap)
				if err != nil {
		...
		// Transforms operate on http.Response, so we wrap the JSON result in a
		// synthetic response, run each transform sequentially, then read the
		// (possibly modified) body back.
		if len(c.transforms) > 0 && jsonResult != nil {
			modifiedJSON, trErr := c.applyTransforms(jsonResult)
			if trErr != nil {
				slog.Error("failed to apply callback transforms", "url", c.URL, "error", trErr)
				return nil, trErr
			}
			jsonResult = modifiedJSON
		}
	}
...
// applyTransforms wraps a JSON value in a synthetic http.Response, runs each
// configured transform handler sequentially, then reads the modified body back
// and returns the updated JSON value.
func (c *Callback) applyTransforms(jsonResult any) (any, error) {
	body, err := json.Marshal(jsonResult)
	if err != nil {
		return nil, fmt.Errorf("callback: failed to marshal JSON for transforms: %w", err)
	}

	// Build a synthetic http.Response so transforms can operate normally.
	...
	for i, handler := range c.transforms {
		if err := handler.Apply(syntheticResp); err != nil {
			return nil, fmt.Errorf("callback: transform[%d] failed: %w", i, err)
		}
	}

	// Read back the (possibly modified) body.
	modifiedBody, err := io.ReadAll(syntheticResp.Body)
	if syntheticResp.Body != nil {
		syntheticResp.Body.Close()
	}
	if err != nil {
		return nil, fmt.Errorf("callback: failed to read transformed body: %w", err)
	}

	var modified any
	if err := json.Unmarshal(modifiedBody, &modified); err != nil {
		return nil, fmt.Errorf("callback: failed to parse transformed JSON: %w", err)
	}
	return modified, nil
}
...
// DoSequentialWithType executes callbacks in order with variable chaining. Each
// synchronous callback's results are merged into a chain context so that
// subsequent callbacks can reference prior results in their body templates
// (e.g., {{on_request_1.token}}). Async callbacks run in background goroutines
// with panic recovery and do not participate in chaining.
func (c *Callbacks) DoSequentialWithType(ctx context.Context, obj any, callbackType string) (map[string]any, error) {
	...
	// Create a chaining context that accumulates results from prior callbacks.
	// Each sync callback's results are merged into chainCtx so subsequent
	// callbacks can reference them in body templates (e.g., {{key1}}).
	// We shallow-copy the original obj to avoid mutating the caller's data.
	chainCtx := shallowCopyObj(obj)
	...
	for i, cb := range *c {
		...
		if cb.Async {
			// Execute asynchronously in background - uses original obj (no chaining for async)
			...
			// Execute synchronously, passing chainCtx so body templates can
			// reference results from prior callbacks.
			timeout := 10 * time.Second
			if cb.Timeout > 0 {
			...
			callbackCtx, cancel := context.WithTimeout(ctx, timeout)
			cbResult, err := cbCopy.Execute(callbackCtx, chainCtx)
			cancel()
			...
			if err != nil {
				...
				// Merge callback results into chainCtx for subsequent callbacks.
				// This enables variable chaining: callback B can reference
				// callback A's results in its body template.
				//
				// cbResult is wrapped as {variableName: innerMap}, so we merge
				// both the wrapped key (for namespaced access like {{cb1.key}})
				// and the inner map values (for direct access like {{key}}).
				for k, v := range cbResult {
					chainCtx[k] = v
					// Also merge inner map values for direct access
...
// shallowCopyObj creates a shallow copy of obj as map[string]any.
// If obj is already a map[string]any, each top-level key is copied into a new map.
// Otherwise the value is wrapped under a "data" key.
// This prevents mutation of the caller's original data when merging callback results.
func shallowCopyObj(obj any) map[string]any {
	if obj == nil {
