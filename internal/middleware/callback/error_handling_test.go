...
func TestCallbackTimeoutErrors(t *testing.T) {
	t.Run("server response timeout via context", func(t *testing.T) {
		server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			time.Sleep(500 * time.Millisecond) // Longer than callback timeout
		...
		// Should timeout around 200ms, not wait for full 500ms
	...
	t.Run("server response timeout via sequential execution", func(t *testing.T) {
		server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			time.Sleep(2 * time.Second) // Longer than callback timeout
			w.WriteHeader(http.StatusOK)
		}))
		defer server.Close()
