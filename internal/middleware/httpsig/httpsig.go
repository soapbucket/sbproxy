// Package httpsig implements RFC 9421 HTTP Message Signatures for
// signing outbound requests and verifying inbound request signatures.
package httpsig
...
func SignRequest(req *http.Request, cfg *Config) error {
...
func VerifyRequestSignature(req *http.Request, cfg *Config) error {
...
func computeSignature(base []byte, cfg *Config) ([]byte, error) {
	switch cfg.Algorithm {
	case AlgHMACSHA256:
...
func verifySignatureBytes(base, sig []byte, cfg *Config) error {
	switch cfg.Algorithm {
	case AlgHMACSHA256:
		h := hmac.New(sha256.New, []byte(cfg.Secret))
		h.Write(base)
...
// Middleware returns an http.Handler middleware that applies RFC 9421
...
// GenerateECDSAKeyPairForTest is a test helper that generates an ECDSA P-256 key pair
