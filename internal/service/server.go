...
func StartHTTP(p ProxyConfig, m manager.Manager, callbackCache *callback.CallbackCache, handler *chi.Mux) error {
	...
	srv := &http.Server{
		...
		ConnContext: func(ctx context.Context, conn net.Conn) context.Context {
			...
			if timingConn, ok := conn.(*fingerprint.ConnectionTiming); ok {
	...
	listener = fingerprint.NewTimingListener(listener)
...
func StartHTTPS(p ProxyConfig, m manager.Manager, callbackCache *callback.CallbackCache, tlsConfig *tls.Config, handler *chi.Mux) error {
	...
	srv := &http.Server{
		...
		ConnContext: func(ctx context.Context, conn net.Conn) context.Context {
			...
			var timingConn *fingerprint.ConnectionTiming
			if tc, ok := conn.(*fingerprint.ConnectionTiming); ok {
				timingConn = tc
			} else if tlsConn, ok := conn.(*tls.Conn); ok {
				if underlying := tlsConn.NetConn(); underlying != nil {
					if tc, ok := underlying.(*fingerprint.ConnectionTiming); ok {
						timingConn = tc
					}
				}
			}
...
func StartHTTP3(p ProxyConfig, m manager.Manager, callbackCache *callback.CallbackCache, tlsConfig *tls.Config, handler *chi.Mux) error {
	...
	srv := &http3.Server{
		...
		ConnContext: func(ctx context.Context, conn *quic.Conn) context.Context {
			...
			// Unified connection context — single context.WithValue instead of 3
			quicTiming := fingerprint.NewQUICConnectionTiming()
			cc := &reqctx.ConnectionContext{
				Manager:          m,
				CallbackCache:    callbackCache,
...
// ShouldBindHTTPSProxy performs the should bind https proxy operation.
func ShouldBindHTTPSProxy(c HTTPSProxyConfig) bool {
	return c.Port > 0 && c.CertificateFile != "" && c.CertificateKeyFile != ""
}

// StartHTTPSProxyServer performs the start https proxy server operation.
func StartHTTPSProxyServer(cfg HTTPSProxyConfig, m manager.Manager) error {
	if !ShouldBindHTTPSProxy(cfg) {
		slog.Debug("HTTPS proxy server disabled")
		return nil
	}

	// Load TLS certificate
	tlsCert, err := tls.LoadX509KeyPair(cfg.CertificateFile, cfg.CertificateKeyFile)
	if err != nil {
		return fmt.Errorf("failed to load HTTPS proxy certificate: %w", err)
	}
	...
	if err != nil {
		return fmt.Errorf("failed to listen on HTTPS proxy port %d: %w", cfg.Port, err)
	}
	defer listener.Close()

	handler := middleware.NewHTTPSProxyHandler(m, cfg.AuthRealm, httpsproxy.ListenerOptions{
		DisableHTTP2Connect:    cfg.DisableHTTP2Connect,
		DisableHTTP3Connect:    cfg.DisableHTTP3Connect,
		EnableRFC8441WebSocket: cfg.EnableRFC8441WebSocket,
		EnableConnectUDP:       cfg.EnableConnectUDP,
		EnableConnectIP:        cfg.EnableConnectIP,
		ConnectUDPTemplate:     cfg.ConnectUDPTemplate,
	})
	...
	go func() {
		<-m.GetServerContext().Done()
		slog.Info("shutting down HTTPS proxy server", "port", cfg.Port)
		shutdownCtx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
		defer cancel()
		if err := server.Shutdown(shutdownCtx); err != nil {
			slog.Error("HTTPS proxy server shutdown error", "error", err, "port", cfg.Port)
		}
	}()

	if !cfg.DisableHTTP3Connect || cfg.EnableConnectUDP || cfg.EnableConnectIP {
		...
		if err != nil {
			return fmt.Errorf("failed to listen for HTTPS proxy HTTP/3 on %s: %w", listenAddr, err)
		}
		...
		go func() {
			slog.Info("HTTPS proxy HTTP/3 server listening",
				"port", cfg.Port,
				"hostname", cfg.Hostname,
				"connect_udp", cfg.EnableConnectUDP)
			if err := http3Server.Serve(packetConn); err != nil && err != http.ErrServerClosed {
				slog.Error("HTTPS proxy HTTP/3 server error", "error", err, "port", cfg.Port)
			}
		}()
	}

	slog.Info("HTTPS proxy server listening",
		"port", cfg.Port,
		"hostname", cfg.Hostname,
		"cert", cfg.CertificateFile)
	if err := server.Serve(listener); err != nil && err != http.ErrServerClosed {
		slog.Error("HTTPS proxy server error", "error", err, "port", cfg.Port)
		return fmt.Errorf("HTTPS proxy server error: %w", err)
	}
