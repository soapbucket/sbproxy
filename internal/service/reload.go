...
// ReloadHook is called after a successful config reload. Hooks run inside the
// ReloadManager's mutex, so they must not block for extended periods.
...
func (rm *ReloadManager) reloadConfiguration() {
	...
	// Fire reload hooks (e.g., compiled config rebuild).
	for _, hook := range rm.hooks {
		hook()
	}
...
// OnReload registers a hook that is called after each successful configuration
// reload. Hooks run synchronously in order of registration. They should be
// fast; long-running work should be dispatched to a goroutine inside the hook.
func (rm *ReloadManager) OnReload(hook ReloadHook) {
	rm.mu.Lock()
	defer rm.mu.Unlock()
