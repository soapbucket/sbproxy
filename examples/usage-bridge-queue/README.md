...
The row stops there, and that is the design rather than a limitation of the fixture. The request path enqueues and returns; the HTTP call to the provider belongs to the worker, behind its own lease and its own idempotency key, so nothing on the request path ever waits on Stripe. Rows here read `queued` and stay that way, because the reporter's API root is a compiled-in `https://api.stripe.com` with no override and this stack has no network.
...
printf '%s' "$(openssl rand -hex 32)" > /tmp/sbproxy-usage-bridge/binding.key
...
The two secret files are named rather than inlined, because an inline credential in a payments field is rejected at load with the field path. `binding.key` serves as the recovery key too: both are 32 bytes of hex, both are demo-only, and the recovery cipher checks the resolved length rather than where it came from.

There is no `docker-compose.yml`, and the reason is the build rather than the stack: `Dockerfile.cloudbuild` compiles the default feature set, so the shared image would start this config and fail on the configured reporter. `scripts/examples-smoke.sh` therefore skips this directory; [`e2e/tests/usage_bridge.rs`](../../e2e/tests/usage_bridge.rs) asserts the same outcomes automatically, starting from an HTTP request and finishing by reading the same SQLite file.
...
One row per billable unit. `status` is the visible half of the split above: a freshly queued row reads `queued`, and only the worker moves it on.

```bash
sqlite3 /tmp/sbproxy-usage-bridge/payments.sqlite3 \
  'select reporter, usage_identifier, tenant_id, origin_id, status, quantity from usage_reports order by created_at_ms'
```
