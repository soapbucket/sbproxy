...
| [ai-budget](ai-budget/) | Two stacked budget limits with `on_exceed: downgrade`. The workspace-wide cap allows up to USD 500 of spend per month and downgrades to `claude-haiku-4-5`. |
...
| [ai-virtual-keys](ai-virtual-keys/) | Two virtual keys, two teams, two budgets. The frontend team's key is allowlisted to `claude-haiku-4-5` only, capped at 30 RPM and 60,000 TPM, w |
