#!/usr/bin/env python3
"""Bounded local OpenAI-shaped fixture for the multi-tenant-saas example.
...
back. Deterministic on purpose: the example demonstrates which tenant
scope a credential resolved from, not model quality. Every tenant in the
example reaches this one upstream, so anything that differs between them
came from the config, not from the provider.
"""
