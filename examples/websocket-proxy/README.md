...
The counter's third reason, `subprotocol_violation`, fires when an upstream selects a subprotocol outside a configured `subprotocols` allowlist; that refusal happens before the tunnel opens, so it renders an ordinary `502` and its record carries `status_code: 502`. The fourth, `control_frame_violation`, fires when a control frame declares more than RFC 6455's 125 payload bytes or arrives fragmented; like the size cap it tears the tunnel down mid-stream with no HTTP status, so its record carries `status_code: 0`. One thing worth alerting on across all four: `sum by (reason, origin) (rate(sbproxy_websocket_teardowns_total[5m]))`.
