...
```bash
make run CONFIG=examples/25-auth-cap/sb.yml
```
...
```bash
python3 examples/25-auth-cap/mint.py > /tmp/cap.jwt
```
...
```bash
$ python3 examples/25-auth-cap/mint.py --aud api.different.com > /tmp/cap-bad-aud.jwt
...
- [examples/20-auth-jwt](../20-auth-jwt) - generic JWT auth (HS256 with a shared secret)
- [examples/23-auth-forward](../23-auth-forward) - delegated auth via a subrequest
