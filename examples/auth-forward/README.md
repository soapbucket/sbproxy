...
The total includes both the auth subrequest to `test.sbproxy.dev/status/200` and the forwarded request to `test.sbproxy.dev/get`.

To see the rejection path, change `url` in the config to `https://test.sbproxy.dev/status/401` and rerun:
