...
The `guardrails.external` entry uses the generic adapter. `name` becomes a safe identifier in logs and client error codes. Metrics use bounded provider, phase, and outcome labels. `url` points at the fixture webhook, and `allow_private_url: true` makes this local test explicit. `mode: pre_call` evaluates the request before any provider call. `default_on: true` automatically enables the input check on this route. `fail_open: false` returns a blocking result if the webhook is unavailable, malformed, slow, or oversized. `timeout_ms: 500` bounds each webhook call.
