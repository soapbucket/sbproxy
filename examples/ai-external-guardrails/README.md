...
The `fixture-model` provider uses `provider_type: openai` because the model fixture accepts OpenAI chat completions. `base_url` points at `127.0.0.1:18080/v1`. `allow_private_base_url: true` is required because loopback targets are blocked by default to prevent server-side request forgery. The model list and `default_model` keep routing deterministic.

The `guardrails.external` entry uses the generic adapter. `name` becomes the safe error code and metric label. `url` points at the fixture webhook, and `allow_private_url: true` makes this local test explicit. `mode: pre_call` evaluates the request before any provider call. `default_on: true` enables it for ordinary AI requests. `fail_open: false` returns a blocking result if the webhook is unavailable, malformed, slow, or oversized. `timeout_ms: 500` bounds each webhook call.
