#!/usr/bin/env python3
"""Bounded local OpenAI-shaped fixture for the ai-predictive-budget example.
...
That last part is what makes the example runnable. Soft landing is a
function of accumulated spend, and spend comes from the completed call's
own `usage` object. Waiting for real traffic to walk a workspace up to
80% of its cap is not a demo, so the request names the tokens it wants
billed:

    {"messages": [{"role": "user", "content": "spend=850"}]}

reports 850 prompt tokens and 0 completion tokens. Anything without a
`spend=` marker reports 1. The number is a demo dial, not a measurement:
nothing here tokenizes anything.

Echoing the dispatched model back is the second half. The soft-landing
downgrade rewrites the model before dispatch, so a fixture that echoes
what it received is what makes the rewrite visible from the client.
"""
...
def self_test():
    ...
    assert requested_spend({"messages": [{"role": "user", "content": "spend=850"}]}) == 850
    ...
    assert (
        requested_spend(
            {
                "messages": [
                    {"role": "system", "content": "no dial here"},
                    {"role": "user", "content": "please spend=7 units"},
                ]
            }
        )
        == 7
