...
**Serve your own.** The same binary runs the models. A provider whose entire body is a `serve:` block with one model line boots to a first token: SBproxy resolves the weights, fits an engine and quant to your GPU (an L4 takes FP8, a T4 falls back to an int4 GGUF), spawns vLLM or llama.cpp as a supervised subprocess, and routes to it. Alias `claude-sonnet-4-5` to a local model and your coding assistant runs on your hardware with a base-URL change. Put a hosted provider after the local one and overflow spills to the cloud under policy, with the ledger reporting what the GPU saved you.
