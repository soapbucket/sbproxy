...
**Govern both.** This is the agent governance layer: virtual keys mint and revoke at runtime, guardrails screen prompts and completions and can redact a streaming response mid-flight, and every request can emit a signed usage receipt you can verify offline. Inbound, SBproxy is an MCP gateway: federate your tools behind OAuth2, verify signed agents (RFC 9421), and meter AI crawlers with HTTP 402 Pay Per Crawl.
...
Weighing the options? See [how SBproxy compares to other LLM gateways: LiteLLM, Portkey, Helicone, and Kong](docs/comparison.md), plus the [benchmark methodology](https://sbproxy.dev/benchmark).
...
**One endpoint for every provider:** the same OpenAI-shaped request answered by OpenAI, Anthropic, and Google; only the host changes. ([config](examples/ai-gateway-quickstart/))

![One OpenAI-compatible request routed to OpenAI, Anthropic, and Google](docs/assets/ai-gateway.gif)
