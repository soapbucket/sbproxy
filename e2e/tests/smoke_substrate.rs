...
//!
//! - Span chain length >= 5: `sbproxy.intake.accept` ->
//!   `sbproxy.policy.enforce` -> `sbproxy.action.challenge|redeem` ->
//!   `sbproxy.ledger.redeem`.
//! - Every log line carries `request_id`, `trace_id`, `span_id`,
//!   `tenant_id`, `agent_id`.
//! - Redemption emits an `audit_emit` envelope with `action:
//!   Redeem|Approve` and a typed target.
//! - `sbproxy_requests_total` carries `agent_id` (registry-bounded),
//!   `agent_class`, `payment_rail`.
//!
