#!/usr/bin/env bash
# generate-certs.sh - Generate self-signed development certificates for testing
#
# Creates CA, server, and client certificates in test/certs/
# Server cert covers localhost, *.test, 127.0.0.1
#
# Usage: ./test/generate-certs.sh
...
openssl req -x509 -newkey rsa:2048 -nodes \
    ...
    -days 3650 \
    -subj "/CN=sbproxy Test CA/O=sbproxy-test" \
    2>/dev/null

# Server cert (localhost + *.test + 127.0.0.1)
echo "[2/3] Generating server certificate..."
openssl req -newkey rsa:2048 -nodes \
    -keyout "$CERT_DIR/server-key.pem" \
    -out "$CERT_DIR/server.csr" \
    -subj "/CN=localhost/O=sbproxy-test" \
    2>/dev/null
...
openssl req -newkey rsa:2048 -nodes \
    ...
    -subj "/CN=test-client/O=sbproxy-test" \
    2>/dev/null
