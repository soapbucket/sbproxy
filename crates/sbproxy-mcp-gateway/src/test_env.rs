...
//! makes both functions `unsafe`. The tests here whose subject genuinely
//! is `MCP_GATEWAY_BASE_URL` (the DPoP startup-validation gate, the
//! well-known route's issuer derivation) go through [`EnvVarGuard`],
//! which serializes every mutation in this binary behind one lock and
//! restores the previous values when it is dropped, panic included.
//! Production code must never call `set_var` / `remove_var` at all;
//! `scripts/check-env-mutation.sh` enforces both rules.
