...
//!    object-storage one, which covers S3, Google Cloud Storage, Azure
//!    Blob, a local directory, and every S3-compatible store reachable
//!    through an `endpoint` override, all through the `object_store`
//!    crate the workspace already carries, with optional AES-256-GCM
//!    sealing before an entry leaves the process.
//!
//!    A second, AWS-SDK-only backend with KMS-wrapped envelope
//!    encryption briefly shipped beside it and was retired in the same
//!    ticket: it reached a strict subset of the same stores through a
//!    second provider SDK and a second code path, for one capability
//!    (per-object data keys wrapped by a cloud KMS) whose absence a
//!    bucket's own SSE-KMS setting covers without putting a KMS round
//!    trip on the read path of a tier whose whole argument is cost.
//!    This matches how Envoy's cache filter is shaped: one pluggable
//!    storage interface, with the provider behind it, rather than one
