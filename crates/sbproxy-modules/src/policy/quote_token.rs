...
///
/// The closed-string fields (`rail`, `shape`) are kept as `String` on the wire
/// type to absorb future closed-enum amendments without breaking parsers
/// that pre-date the addition.
...
///
/// The variants map directly to the closed error codes for the
/// verification path. The ledger service
...
/// Maximum tolerated future-clock skew on `iat`. The 5-minute window is the
/// closed verification-path bound shared by the ledger service.
