...
;;
;; This is a copy of the fixture at
;; crates/sbproxy-extension/src/wasm/testdata/echo.wat, kept here too
;; so the `WasmTransform` unit tests in
;; crates/sbproxy-modules/src/transform/mod.rs (WOR-2493 item 5, the
;; `request_context: true` ctx channel) can prove stdin passes through
;; untouched whether or not `request_context` is set, without crossing
;; a crate boundary for test-only bytes. It is hand-written WAT so the
...
;; behavior written more idiomatically; this WAT version is purely a
