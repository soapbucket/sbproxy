//! Backend-agnostic contract tests for governance reservation accounting.
//!
//! These exercise the [`GovernanceStore`] contract through the in-memory
//! store, which every backend must match: renewal keeps a lease alive
//! without double-charging, terminal states refuse renewal, duplicate
//! terminal operations return the original result, and integer overflow
//! rejects without partial mutation. The Redis tier proves the same
//! contract against a live backend in `governance_redis.rs`.
...
async fn renewal_keeps_a_long_running_ingress_reservation_active_without_double_charge() {
    ...
    assert_eq!(renewed.reserved, reservation.reserved);
