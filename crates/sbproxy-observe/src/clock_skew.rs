...
mod tests {
    ...
    fn clock_skew_metric_gauge_present() {
        ...
        let mf = metric_families
            .iter()
            .find(|mf| mf.name() == "sbproxy_clock_skew_seconds")
            .unwrap();
        // prometheus 0.14 wraps the gauge in `protobuf::MessageField`
        // (instead of returning it by value). Access via `.value` and
        // dereference through `as_ref()`. The `value` field is
        // `Option<f64>` in protobuf 3.x.
        let v = mf.get_metric()[0].gauge.as_ref().unwrap().value.unwrap();
