...
pub const METRICS: &[MetricCapability] = &[
    ...
    MetricCapability {
        ...
        description: "MCP OAuth enforcement decisions that no HTTP status alone reports: the resource server's 401, the per-operation scope refusal and its fail-open twin, the /authorize and /par limiter, the session-capacity refusal, the AS-metadata stale fallback, and the device-consent CSRF refusal.",
        dead_reason: None,
    },
    MetricCapability {
