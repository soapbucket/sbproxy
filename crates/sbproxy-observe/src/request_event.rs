...
pub struct RequestEvent {
    ...
    /// Session linkage. ULID, optionally auto-generated for anonymous
    /// traffic when configured.
    #[serde(skip_serializing_if = "Option::is_none", default)]
    pub session_id: Option<Ulid>,

    /// Caller-supplied parent session linkage; the proxy validates
    /// ULID format but does not check existence.
    #[serde(skip_serializing_if = "Option::is_none", default)]
    pub parent_session_id: Option<Ulid>,

    /// User identifier. Resolved via header / JWT / forward-auth
