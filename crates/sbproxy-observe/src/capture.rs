...
//!   `X-Sb-Property-*` headers, enforces caps, applies redaction.
//! * Session linkage (T2.1): [`capture_session_id`] /
//!   [`capture_parent_session_id`] read `X-Sb-Session-Id` and
//!   `X-Sb-Parent-Session-Id`, validate ULID format, and auto-generate
//!   when configured.
//! * User identity (T3.1): [`capture_user_id`] resolves `X-Sb-User-Id`
//!   / JWT `sub` / forward-auth header in precedence order.
//!
...
/// drop counts, applying caps and redaction.
///
/// Behavior:
///
