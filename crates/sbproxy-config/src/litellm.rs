...
pub fn translate_litellm(input: &str) -> Result<LitellmTranslation> {
    ...
    // Callback lists: known sinks are handled by the usage-sink config; Python
    // module-path callbacks need manual rewrite, so warn on those.
    for cb in cfg
        .litellm_settings
        .callbacks
        .iter()
        .chain(&cfg.litellm_settings.success_callback)
        .chain(&cfg.litellm_settings.failure_callback)
    {
        if !KNOWN_SINK_CALLBACKS.contains(&cb.as_str()) {
            warnings.push(format!(
    ...
    if let Some(strategy) = &cfg.router_settings.routing_strategy {
        action.insert(
            "routing".into(),
            json!(map_routing_strategy(strategy, &mut warnings)),
        );
    }
    ...
    Ok(LitellmTranslation { sb_yaml, warnings })
}

/// Map a LiteLLM `routing_strategy` name to the nearest sbproxy strategy.
/// Unknown strategies warn and fall back to `round_robin`.
fn map_routing_strategy(name: &str, warnings: &mut Vec<String>) -> String {
    match name {
        "simple-shuffle" => "round_robin",
        "latency-based-routing" => "lowest_latency",
        "usage-based-routing" | "usage-based-routing-v2" => "least_token_usage",
        "least-busy" => "least_connections",
