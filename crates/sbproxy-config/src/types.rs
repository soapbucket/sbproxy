...
/// a passing verdict moves the `lkg` pointer
/// ([`crate::revision_store::RevisionStore::record_soak_verdict`]).
///
/// # The verdict is three-way
///
/// Modelled on Argo Rollouts' analysis runs, which complete
...
/// [`Self::min_requests`] rather than reporting a failure. One
...
/// fallback ([`ConfigBootConfig`]) with nothing to boot from. So this
...
/// ([`SyntheticProbeConfig`]), which the soak also reads. The synthetic
...
pub struct ConfigBootConfig {
    ...
    /// to [`BootFallbackMode::Off`], so today's behavior is unchanged
