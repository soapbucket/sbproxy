...
pub enum ConfigSource {
    ...
    Git {
        ...
        ///
        /// Off by default, and deliberately. In the ordinary GitOps
        /// shape the repository *is* the operator's config: the local
        /// file is a pointer, `env:` is the documented spelling for the
        /// cluster shared key, and the Kubernetes operator generates
        /// it. Sealing that by default would leave those nodes with no
        /// legal spelling for a secret at all, because the refusal's
        /// remedy (declare the value in the config the operator owns)
        /// has nowhere to go on this path. Turn it on when the
        /// repository is written by somebody other than whoever runs
        /// this proxy; the document then gets the same treatment a
        /// config-authority bundle gets. See the `Confined fragments`
