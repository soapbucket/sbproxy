...
- **An exact pinned toolchain version** across every lane. We build on a fixed `stable` channel and declare an MSRV, but do not commit a `rust-toolchain.toml` pinning one version. The nightly-only fuzz harnesses used to be the reason. They no longer run in CI at all: they run from the local development gate on a cadence, so nothing in the release pipeline builds on nightly. Converging every lane onto a single pinned toolchain remains part of the bit-for-bit work above.
